...
Public Class CompCatProveedor
    ...
    Private Sub CompCatProveedor_Load(ByVal sender As System.Object, ByVal e As System.EventArgs) Handles MyBase.Load
        ...
        TabControl4.TabPages.RemoveByKey(TabPage3.Name)
...
#Region "Validaciones"
    ...
    Private Sub Proveedor_Modificado(ByVal sender As Object, ByVal e As System.EventArgs) Handles Proveedor.Modificado
        If PuedoRefrescar Then
            ...
            If IsNothing(Proveedor.UltimaCompraObj) Then
                txtImporteUltimaCompra.Text = (0).ToString("N2")
                dtpFechaUltimaCompra.Value = Date.Now
            Else
                txtImporteUltimaCompra.Text = Proveedor.UltimaCompraObj.Total.ToString("N2")
                dtpFechaUltimaCompra.Value = Proveedor.UltimaCompraObj.FechaRecepcion
            End If

            ' txtImporteUltimaCompra.Text = Proveedor.UltimaCompraObj.Total.ToString("N2")
            'dtpFechaUltimaCompra.Value = Proveedor.UltimaCompraObj.FechaRecepcion
            If IsNothing(Proveedor.UltimaPagoObj(Controlador.Sesion.MiEmpresa.Empndx)) Then
                txtImporteUltimoPago.Text = (0).ToString("N2")
            Else
                txtImporteUltimoPago.Text = Proveedor.UltimaPagoObj(Controlador.Sesion.MiEmpresa.Empndx).Importe.ToString("N2")
            End If

            ''txtImporteUltimaCompra.Text = IIf(IsNothing(Proveedor.UltimaPagoObj), (0).ToString("N2"), Proveedor.UltimaPagoObj.Importe.ToString("N2"))


            'dtpFechaUltimoPago.Value = IIf(IsNothing(Proveedor.UltimaPagoObj), Date.Now, Proveedor.FechaUltimoPago)
            If IsNothing(Proveedor.UltimaPagoObj(Controlador.Sesion.MiEmpresa.Empndx)) Then
                dtpFechaUltimoPago.Value = Date.Now
            Else
                dtpFechaUltimoPago.Value = Proveedor.UltimaPagoObj(Controlador.Sesion.MiEmpresa.Empndx).FechaPago
            End If

            txtComprasAnoAnterior.Text = Proveedor.ComprasAnioAnterior
            txtComprasAnoActual.Text = Proveedor.ComprasAnioActual
    ...
    Private Function Validar2() As Boolean
        Try
            If txtNombre.Text = String.Empty Then
                ...
                Return False
            End If
            If TxtRazonSocial.Text = "" Or TxtRFC.Text = "" Then
...
#End Region
    ...
    Private Sub CompCatProveedor_KeyDown(sender As System.Object, e As System.Windows.Forms.KeyEventArgs) Handles MyBase.KeyDown
