...
#Region "Métodos"
    Public Sub Limpiar()
        ...
        Me.TxtSubtotal.Text = CDec(0).ToString("C2")
        Me.txtIva.Clear()
        Me.TxtAnticipo.Text = CDec(0).ToString("C2")
        Me.TxtTotal.Clear()
    ...
    Function ObtenerRecepcionesCompra() As Boolean
        Try
            If Me.Editar = False Then
                ...
                'Recep.Obtener(Me.CmbProveedor.SelectedValue, "", ClasesNegocio.CondicionEnum.ACTIVOS)
                'MessageBox.Show(Recep.Count.ToString())
                If Recep.Obtener(Me.CmbProveedor.SelectedValue, "", ClasesNegocio.CondicionEnum.ACTIVOS) > 0 Then
                    ...
                    If Recep.Count > 0 Then
                        For i As Integer = 0 To Recep.Count - 1
                            ...
                            Me.DgvRecepciones.Rows(i).Cells("FolioOrden").Value = Recep(i).NoRemision
...
#End Region
    ...
    Private Sub DgvRecepciones_CellContentClick(ByVal sender As Object, ByVal e As System.Windows.Forms.DataGridViewCellEventArgs) Handles DgvRecepciones.CellContentClick
        ...
        '    If Me.DgvRecepciones.Rows(e.RowIndex).Cells("Agregar").Value IsNot Nothing Then
    ...
    Private Sub DgvRecepciones_CurrentCellDirtyStateChanged(sender As System.Object, e As System.EventArgs) Handles DgvRecepciones.CurrentCellDirtyStateChanged
    ...
    Private Sub DgvRecepciones_CellValueChanged(sender As System.Object, e As System.Windows.Forms.DataGridViewCellEventArgs) Handles DgvRecepciones.CellValueChanged
        If e.ColumnIndex = Me.Agregar.Index And bandera Then
            If Me.DgvRecepciones.Rows(e.RowIndex).Cells("Agregar").Value IsNot Nothing Then
                'If CBool(Me.DgvRecepciones.Rows(e.RowIndex).Cells("Agregar").Value) Then
                '    Me.DgvRecepciones.Rows(e.RowIndex).Cells("Agregar").Value = False
                'Else
                ...
                For i As Integer = 0 To Me.DgvRecepciones.Rows.Count - 1
                    If CBool(Me.DgvRecepciones.Rows(i).Cells("Agregar").Value) Then
                        ...
                        If Editar = False Then
                            'Dim total As Decimal
                            ' total = Re.Precio * Re.Cantidad
                            Subtotal = Subtotal + Decimal.Parse(Me.DgvRecepciones.Rows(i).Cells("Importe").Value.ToString.Replace("$", ""))
                        End If
                        'End If
                    End If
                Next
