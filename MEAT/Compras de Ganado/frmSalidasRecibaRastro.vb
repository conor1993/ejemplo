...
Public Class frmSalidasRecibaRastro
...
#Region "Metodos"
    Public Sub Inicializar_Forma()
        ...
        Else
            ...
            Me.txtTotalCabezas.Top = 473
            Me.txtTotalImporte.Top = 473
            'Me.txtTotalImporteVenta.Top = 473
            Me.txtTotalKilos.Top = 473
    ...
    Private Sub ValidarDatos()
        ...
        ElseIf cmbRastro.SelectedValue Is Nothing Then
    ...
    Private Function VerificarAretes(ByVal Arete As String, ByVal Columna As Integer, ByVal Renglon As Integer, ByVal TipodeGanado As Integer) As Boolean
        Try
            For i As Integer = Me.dgvDetalleLote.Rows.Count - 1 To 0 Step -1
                If i <> Renglon Then
                    If Me.dgvDetalleLote.Rows(i).Cells(Columna).Value = Arete Then
                        ...
                        'Identificador = Me.dgvDetalleLote.Rows(Renglon).Cells(Me.clmIdentificador.Index).Value
                        'TipodeGanado = Me.dgvDetalleLote.Rows(Renglon).Cells(Me.clmTipoGanado.Index).Value

                        'Me.dgvDetalleLote.Rows.RemoveAt(Renglon)
...
#Region "Acciones_Menu"
    ...
    Public Function Buscar() As Boolean
        ...
        Dim frmCorralesReciba As New frmBusquedaCorralesRecibaRastro
        ...
        If frmCorralesReciba.ShowDialog() = Windows.Forms.DialogResult.OK Then
            ...
            Else
                ...
                cmbRastro.SelectedValue = CorralReciba.IdRastro
    ...
    Public Function Guardar(ByVal Trans As HC.Transaction) As Boolean
        ...
        If Me.Validate() Then
            If Controlador.Configuracion.Engorda.RequerirArete Then
                ...
                CorralRecibaAretes.IdRastro = Me.cmbRastro.SelectedValue
                CorralRecibaAretes.IdTipoReciba = SalidaGanadoCabeceroClass.TipoSalida.Corral_de_Reciba_a_Rastro
                ...
                Next
                Controlador.RealizarSalidadeRecibaaRastro(CorralRecibaAretes, Me.dtpFecha.Value, Trans)
            Else
                Controlador.RealizarSalidadeRecibaaRastro(CorralReciba, Me.dtpFecha.Value, Trans)
            End If
            Trans.Commit()
            If Controlador.Configuracion.Engorda.RequerirArete Then
                MessageBox.Show("La Salida de Reciba a Rastro se ha realizado satisfactoriamente con el folio: " & CorralRecibaAretes.Folio)
                Controlador.ImpresionSalidaDeGanado(CorralRecibaAretes)
            Else
                MessageBox.Show("La Salida de Reciba a Rastro se ha realizado satisfactoriamente con el folio: " & CorralReciba.Folio)
    ...
    Public Function Borrar(ByVal Trans As HC.Transaction) As Boolean
        ...
        Else
            Controlador.CancelarSalidadeRecibaaRastro(CorralReciba, CorralReciba.Detalles, Trans)
        End If
        Trans.Commit()
        Cancelar()
        MessageBox.Show("La Salida de Reciba a Rastro se ha cancelado satisfactoriamente")
        Return True
    End Function
...
#Region "Eventos"
    ...
    Private Sub cmbRastro_SelectedValueChanged(ByVal sender As System.Object, ByVal e As System.EventArgs) Handles cmbRastro.SelectedValueChanged
        If cmbRastro.SelectedValue IsNot Nothing And (Estado = FormState.Nuevo Or Estado = FormState.Editar) Then
            CorralReciba.IdRastro = cmbRastro.SelectedValue
        End If
    End Sub
