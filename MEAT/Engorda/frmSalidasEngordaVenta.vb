...
Public Class frmSalidasEngordaVenta
    Inherits MeatForm
...
#Region "Metodos"
    ...
    Private Sub ActualizarSeleccion(ByRef RenglonLote As DataGridViewRow)
        For Each GridRow As DataGridViewRow In dgvDetalleLote.Rows
            ...
            If dr.Length > 0 Then
                ...
                GridRow.Cells(clmPrecioxKilo.Index).Value = dr(0).Item("PrecioxKilo")
            End If
        Next
    End Sub
    ...
    Private Sub RecalcularMontosGenerales()
        Dim TotalCabezas As Integer = 0D, TotalKilos As Decimal = 0D, TotalImporte As Decimal = 0D, TotalImporteVenta As Decimal = 0D
        ...
        For Each GridRow As DataGridViewRow In dgvLotes.Rows
            If Not GridRow.IsNewRow Then
                ...
                TotalImporte += Val(GridRow.Cells(clmImporte.Index).Value.ToString().Replace("$", "").Replace(",", ""))

                If IsNumeric(GridRow.Cells(clmImporteVenta.Index).Value) Then
                    TotalImporteVenta += CDec(GridRow.Cells(clmImporteVenta.Index).Value.ToString())
                End If
            End If
        Next
        ...
        txtTotalImporteVenta.Text = TotalImporteVenta.ToString("C2")
        dgvLotes.Refresh()
    End Sub

    Private Sub RecalcularDetalleLotes(Optional ByRef GridRow As DataGridViewRow = Nothing, Optional ByVal KilosModificados As Boolean = False, Optional ByVal ImporteVentaModificado As Boolean = False)
        Dim Importe As Decimal = 0D, Kilos As Decimal = 0D, ImporteVenta As Decimal = 0D
        ...
        For Each detalle As dsSeleccionDetalleLote.SelDetalleLoteRow In GridRow.Tag.Rows
            ...
            ImporteVenta += detalle.PrecioxKilo * detalle.Kilos
        Next
        ...
        If ImporteVentaModificado AndAlso GridRow.Tag.Rows.Count > 0 Then
            GridRow.Cells(clmImporteVenta.Index).Value = ImporteVenta
        End If
    ...
    Public Sub FinDeEdicion(ByVal e As System.Windows.Forms.DataGridViewCellEventArgs)
        ...
        ElseIf e.ColumnIndex = clmKilos.Index Then
            ...
            If Kilos > 0D Then
                dgvLotes.Rows(e.RowIndex).Cells(clmImporteVenta.Index).ReadOnly = False
            Else
                dgvLotes.Rows(e.RowIndex).Cells(clmImporteVenta.Index).ReadOnly = True
                dgvLotes.Rows(e.RowIndex).Cells(clmImporteVenta.Index).Value = 0D
            End If

            RecalcularDetalleLotes(Nothing, True)
            RecalcularMontosGenerales()
        ElseIf e.ColumnIndex = clmImporteVenta.Index Then
            Dim Kilos As Decimal = CType(dgvLotes.Rows(e.RowIndex).Cells(clmKilos.Index).Value, Decimal)
            Dim ImporteVenta As Decimal = CType(dgvLotes.Rows(e.RowIndex).Cells(clmImporteVenta.Index).Value, Decimal)
            Dim ImporteVentaxCabeza As Decimal = 0D
            Dim Lote As LoteClass = CType(dgvLotes.Rows(e.RowIndex).Cells(clmLote.Index).Tag, LoteClass)

            If Kilos <> 0D Then
                ImporteVentaxCabeza = ImporteVenta / Kilos
            End If

            For Each GridRow As DataGridViewRow In dgvDetalleLote.Rows
                If GridRow.Cells(clmSeleccion.Index).Value Then
                    GridRow.Cells(clmPrecioxKilo.Index).Value = ImporteVentaxCabeza
                    dgvDetalleLote_CellEndEdit(chkTodos, New DataGridViewCellEventArgs(clmPrecioxKilo.Index, GridRow.Index))
                End If
            Next
            RecalcularDetalleLotes(Nothing, False, True)
            RecalcularMontosGenerales()
        End If
    End Sub
...
#Region "Acciones_Menu"
    ...
    Public Overrides Function Buscar() As Boolean
        Dim frmCorralesReciba As New frmBusquedaCorralesAVenta
        ...
        If frmCorralesReciba.ShowDialog() = Windows.Forms.DialogResult.OK Then
            ...
            For Each Item As dsLotesTiposGanado.LoteTipoGanadoRow In SalidaGanado.Lotes_y_TiposGanado
                Dim index As Integer = dgvLotes.Rows.Add(New LoteClass(Item.IdLote), Item.CorralDescripcion, New TipoGanadoClass(Item.IdTipoGanado), Item.Cabezas, Item.Kilos, Item.Importe, Item.ImporteVenta)
            ...
            txtTotalImporteVenta.Text = SalidaGanado.ImporteVenta.Value.ToString("C2")
    ...
    Public Overrides Function Limpiar() As Boolean
        ...
        txtTotalImporteVenta.ResetText()
        txtFolio.ResetText()
        RemoveHandler chkTodos.CheckStateChanged, AddressOf chkTodos_CheckedChanged
        chkTodos.Checked = False
        AddHandler chkTodos.CheckStateChanged, AddressOf chkTodos_CheckedChanged
    ...
    Public Overrides Function Guardar(ByVal Trans As HC.Transaction) As Boolean
        ...
        If MessageBox.Show("La Salida por Venta se efectuara, ¿Esta seguro?", Controlador.Sesion.MiEmpresa.Empnom, MessageBoxButtons.YesNo, MessageBoxIcon.Question) = Windows.Forms.DialogResult.No Then
            Return False
        End If
            ...
            For Each Detalle As DataGridViewRow In dgvLotes.Rows
                If Not Detalle.IsNewRow AndAlso Detalle.Tag IsNot Nothing AndAlso Detalle.Tag.Rows.Count > 0 Then
                    For Each Renglon As dsSeleccionDetalleLote.SelDetalleLoteRow In Detalle.Tag.Rows
                        ...
                        Renglon.IdMceCatLotesDet, "", "", "", Renglon.PrecioxKilo))
                    Next
                    dsDetalle.Merge(CType(Detalle.Tag, dsSeleccionDetalleLote.SelDetalleLoteDataTable))
                End If
            Next
            FrmVistaDetalle = New frmDetalleLote(dsDetalle, True)

            If FrmVistaDetalle.ShowDialog() = Windows.Forms.DialogResult.OK Then
                Controlador.RealizarSalidadeCorralaVenta(SalidaGanado, Me.dtpFecha.Value, Trans)
                Trans.Commit()
                MessageBox.Show("La Salida de Corral a Venta se ha realizado satisfactoriamente con el folio: " & SalidaGanado.Folio)
    ...
    Public Overrides Function Borrar(ByVal Trans As HC.Transaction) As Boolean
        If MessageBox.Show("La Salida por Venta (" & SalidaGanado.Folio & ") se cancelara, ¿Esta seguro?", Controlador.Sesion.MiEmpresa.Empnom, MessageBoxButtons.YesNo, MessageBoxIcon.Question) = Windows.Forms.DialogResult.No Then
            Return False
        End If

        Controlador.CancelarSalidaDeCorralAVenta(SalidaGanado, Trans)
        Trans.Commit()
        Cancelar()
        MessageBox.Show("La Salida de Corral a Venta se ha cancelado satisfactoriamente")
        Return True
    End Function
        ...
        If e.ColumnIndex = clmSeleccion.Index Then
            ...
            If Celda.EditedFormattedValue Then
                ...
                If Not dr.Length > 0 Then
                    Dim PrecioxKilo As Decimal = 0D

                    If IsNumeric(dgvDetalleLote.Rows(e.RowIndex).Cells(clmPrecioxKilo.Index).Value) Then
                        PrecioxKilo = CDec(dgvDetalleLote.Rows(e.RowIndex).Cells(clmPrecioxKilo.Index).Value)
                    End If
            ...
            Else
                ...
                dgvDetalleLote.Rows(e.RowIndex).Cells(clmPrecioxKilo.Index).Value = 0D
            ...
            If Not dgvLotes.CurrentRow.Cells(clmKilos.Index).ReadOnly AndAlso IsNumeric(dgvLotes.CurrentRow.Cells(clmKilos.Index).Value) AndAlso dgvLotes.CurrentRow.Cells(clmKilos.Index).Value > 0 Then
                ...
                Dim Cabezas As Integer = dsDetalles.Rows.Count, KilosxCabeza As Decimal = 0D
            ...
            If Not dgvLotes.CurrentRow.Cells(clmImporteVenta.Index).ReadOnly AndAlso IsNumeric(dgvLotes.CurrentRow.Cells(clmImporteVenta.Index).Value) AndAlso dgvLotes.CurrentRow.Cells(clmImporteVenta.Index).Value > 0 Then
                Dim ImporteVenta As Decimal = CType(dgvLotes.CurrentRow.Cells(clmImporteVenta.Index).Value, Decimal)
                Dim Kilos As Decimal = CType(dgvLotes.CurrentRow.Cells(clmKilos.Index).Value, Decimal)
                Dim Cabezas As Integer = dsDetalles.Rows.Count, PrecioxKilo As Decimal = 0D

                If Kilos <> 0 Then
                    PrecioxKilo = ImporteVenta / Kilos
                End If

                For Each Arete As DataGridViewRow In dgvDetalleLote.Rows
                    If CType(Arete.Cells(clmSeleccion.Index), DataGridViewCheckBoxCell).EditingCellFormattedValue Then
                        Arete.Cells(clmPrecioxKilo.Index).Value = PrecioxKilo
                        dgvDetalleLote_CellEndEdit(chkTodos, New DataGridViewCellEventArgs(clmPrecioxKilo.Index, Arete.Index))
                    End If
                Next
            End If
            ...
            If dgvLotes.CurrentRow IsNot Nothing Then
                ...
                Else
                    If dgvLotes.CurrentRow.Tag IsNot Nothing Then
                        Dim dsDetalles As dsSeleccionDetalleLote.SelDetalleLoteDataTable = CType(dgvLotes.CurrentRow.Tag, dsSeleccionDetalleLote.SelDetalleLoteDataTable)
                        Dim dr() As DataRow = dsDetalles.Select("IdMceCatLotesDet = '" & dgvDetalleLote.Rows(e.RowIndex).Cells(clmIdMceCatLotesDet.Index).Value.ToString & "'")
                        Dim PrecioxKilo As Decimal = 0D
                        ...
                        If IsNumeric(dgvDetalleLote.Rows(e.RowIndex).Cells(clmPrecioxKilo.Index).Value) AndAlso dgvDetalleLote.Rows(e.RowIndex).Cells(clmPrecioxKilo.Index).Value > 0D Then
                            PrecioxKilo = CDec(dgvDetalleLote.Rows(e.RowIndex).Cells(clmPrecioxKilo.Index).Value)
                        Else
                            Dim Kilos As Decimal = CType(dgvLotes.CurrentRow.Cells(clmKilos.Index).Value, Decimal)
                            Dim ImporteVenta As Decimal = CType(dgvLotes.CurrentRow.Cells(clmImporteVenta.Index).Value, Decimal)
                            ...
                            If Kilos <> 0 Then
                                PrecioxKilo = ImporteVenta / Kilos
                                dgvDetalleLote.Rows(e.RowIndex).Cells(clmPrecioxKilo.Index).Value = PrecioxKilo
                            End If
                        End If

                        If dr.Length > 0 Then
                            dr(0).Item("Kilos") = CDec(dgvDetalleLote.Rows(e.RowIndex).Cells(clmDetalleKilos.Index).Value)
                            dr(0).Item("PrecioxKilo") = PrecioxKilo
                        Else
                ...
                End If
                If sender.GetType() IsNot GetType(CheckBox) Then
                    RecalcularDetalleLotes(dgvLotes.CurrentRow, True, True)
                End If
            End If
            If sender.GetType() IsNot GetType(CheckBox) Then
                RecalcularMontosGenerales()
            End If
        ElseIf e.ColumnIndex = clmPrecioxKilo.Index Then
            If dgvLotes.CurrentRow IsNot Nothing Then
                Dim dr() As dsSeleccionDetalleLote.SelDetalleLoteRow = dgvLotes.CurrentRow.Tag.Select("IdMceCatLotesDet = '" & dgvDetalleLote.Rows(e.RowIndex).Cells(clmIdMceCatLotesDet.Index).Value.ToString & "'")

                If dr.Length > 0 Then
                    dr(0).PrecioxKilo = CDec(dgvDetalleLote.Rows(e.RowIndex).Cells(clmPrecioxKilo.Index).Value)
                End If
                If sender.GetType() IsNot GetType(CheckBox) Then
                    RecalcularDetalleLotes(dgvLotes.CurrentRow, False, True)
                    RecalcularMontosGenerales()
                End If
            End If
        End If
    ...
    Private Sub dgvLotes_RowEnter(ByVal sender As System.Object, ByVal e As System.Windows.Forms.DataGridViewCellEventArgs) Handles dgvLotes.RowEnter
        ...
        ElseIf Estado = FormState.Buscar Then
            ...
            For Each Detalle As SalidaGanadoDetalleClass In SalidaGanado.Aretes(CType(dgvLotes.Rows(e.RowIndex).Cells(clmLote.Index).Tag, Integer), CType(dgvLotes.Rows(e.RowIndex).Cells(clmIdTipoGanado.Index).Tag, Integer))
                dgvDetalleLote.Rows.Add(True, Detalle.IdMcecatLotesDet, Detalle.AreteTransporte, Detalle.AreteSiniiga, Detalle.Arete, Detalle.Kilos, Detalle.PrecioxKilo, Detalle.CostoxKilo, Detalle.McgcatTiposdeGanado.Descripcion)
            Next
        End If
    End Sub
