...
Imports EST = ClasesNegocio.AplicacionFormulaClass.EstatusAplicacionFormula

Public Class frmBusquedaAplicacionFormula
    ...
    Public Property AplicacionFormula() As AplicacionFormulaClass
        Get
            Return Me.m_AplicacionFormula
        End Get
        Set(ByVal value As AplicacionFormulaClass)
            Me.m_AplicacionFormula = value
        End Set
    End Property
    ...
    Private Sub btnAceptar_Click(ByVal sender As System.Object, ByVal e As System.EventArgs) Handles btnAceptar.Click
        If Me.dgResultado.SelectedRows.Count > 0 Then
            Me.AplicacionFormula = _
                    New AplicacionFormulaClass(Me.dgResultado.SelectedRows(0).Cells(Me.FolAplFDataGridViewTextBoxColumn.Index).Value.ToString)
    ...
    Private Sub mtb_ClickBuscar(ByVal sender As System.Object, ByVal e As System.Windows.Forms.ToolBarButtonClickEventArgs, ByRef Cancelar As System.Boolean) Handles mtb.ClickBuscar
        Try
            ...
            If Me.txtFolio.Text.Trim.Length > 0 Then
                filtro.Add(HC.VwBusquedaAplicacionFormulaFields.FolAplF Mod String.Format("%{0}%", Me.txtFolio.Text.Trim))
            Else
                If Me.cmbAlmacen.SelectedIndex > -1 Then
                    filtro.Add(HC.VwBusquedaAplicacionFormulaFields.IdAlmacen = CInt(Me.cmbAlmacen.SelectedValue))
                End If

                If Me.chkFiltrar.Checked Then
                    filtro.Add(New OC.FieldBetweenPredicate(HC.VwBusquedaAplicacionFormulaFields.FechaAplicacion, _
                                Me.dtFechaInicio.Value.ToShortDateString, Me.dtFechaFin.Value.AddDays(1).ToShortDateString))
                End If

                If Me.chkVigente.Checked Then
                    filtroEstatus.Add(HC.VwBusquedaAplicacionFormulaFields.Estatus = EST.VIGENTE)
                End If

                If Me.chkContabilizado.Checked Then
                    If filtroEstatus.Count = 0 Then
                        filtroEstatus.Add(HC.VwBusquedaAplicacionFormulaFields.Estatus = EST.CONTABILIZADA)
                    Else
                        filtroEstatus.AddWithOr(HC.VwBusquedaAplicacionFormulaFields.Estatus = EST.CONTABILIZADA)
                    End If
                End If

                If Me.chkCancelado.Checked Then
                    If filtroEstatus.Count = 0 Then
                        filtroEstatus.Add(HC.VwBusquedaAplicacionFormulaFields.Estatus = EST.CANCELADA)
                    Else
                        filtroEstatus.AddWithOr(HC.VwBusquedaAplicacionFormulaFields.Estatus = EST.CANCELADA)
                    End If
                End If
            ...
            Me.aplicacionesFormulas.Clear()
            Me.aplicacionesFormulas.Fill(0, Nothing, False, filtro)
        Catch ex As Exception
            MsgBox(ex.Message, MsgBoxStyle.Critical, "Error")
        End Try
    End Sub
