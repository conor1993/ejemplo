...
Public Class frmFacturacionVentaCorral
...
#Region "Metodos"
    ...
    Private Function Imprimir(ByVal DomicilioCliente As DomicilioClienteClass, Optional ByVal conRFC As Boolean = True) As Boolean
        Try
            ...
            Fact = Controlador.ObtenerFacturasDeVenta(Me.txtFolioFactura.Text, Nothing, 0, EstatusChar.TODOS, TipoFacturaEnum.FACTURACION_DE_VENTA_DE_CORRAL)
    ...
    Private Function CargaSalidasdeGanadoaVenta() As Boolean
        Try
            ...
            If Consultas.ShowDialog = Windows.Forms.DialogResult.OK Then
                ...
                Me.dgvAgruparGanados.DataSource = SalidadeRecibaAVenta.Detalles
                TipodeGanado = SalidadeRecibaAVenta.Detalles.Item(0).IdTipoGanado

                For Each Detalle As SalidaGanadoDetalleClass In SalidadeRecibaAVenta.Detalles
                    If TipodeGanado = Detalle.IdTipoGanado Then
                        Cabezas += Detalle.Cabezas
                        Kilos += Detalle.Kilos
                        Importe += Detalle.Importe
                        ImporteVenta += Detalle.ImporteVenta
                        Me.dgvDetalle.Rows(Renglon).Cells(Me.Codigo.Index).Value = Detalle.IdTipoGanado
                        Me.dgvDetalle.Rows(Renglon).Cells(Me.TipoGanado.Index).Value = Detalle.TipoGanado
                        Me.dgvDetalle.Rows(Renglon).Cells(Me.Cabezas.Index).Value = Cabezas
                        Me.dgvDetalle.Rows(Renglon).Cells(Me.Kilos.Index).Value = Kilos
                        'Me.dgvDetalle.Rows(Renglon).Cells(Me.Importe.Index).Value = ImporteVenta
                        'Me.dgvDetalle.Rows(Renglon).Cells(Me.i.Index).Value = Detalle.TipoGanado.Descripcion
                    Else
                        Renglon += 1
                        Me.dgvDetalle.Rows.Add()
                        TipodeGanado = Detalle.IdTipoGanado
                        Cabezas = Detalle.Cabezas
                        Kilos = Detalle.Kilos
                        Importe = Detalle.Importe
                        ImporteVenta = Detalle.ImporteVenta
...
#Region "Eventos"
    ...
    Private Sub frmFacturacionVentaCorral_Load(ByVal sender As System.Object, ByVal e As System.EventArgs) Handles MyBase.Load
        Try
    ...
    Private Sub MEAToolBar1_ClickBuscar(ByVal sender As Object, ByVal e As System.Windows.Forms.ToolBarButtonClickEventArgs, ByRef Cancelar As Boolean) Handles MEAToolBar1.ClickBuscar
        Try
            ...
            Dim ListaSalidas As New List(Of String)
            Consultas.TipoFactura = TipoFacturaEnum.FACTURACION_DE_VENTA_DE_CORRAL
            If Consultas.ShowDialog = Windows.Forms.DialogResult.OK Then
    ...
    Private Sub MEAToolBar1_ClickNuevo(ByVal sender As Object, ByVal e As System.Windows.Forms.ToolBarButtonClickEventArgs, ByRef Cancelar As Boolean) Handles MEAToolBar1.ClickNuevo
        ...
        If Me.CargaSalidasdeGanadoaVenta() Then
            Habilitar()
    ...
    '                    Importe += Detalle.Importe
    '                    ImporteVenta += Detalle.ImporteVenta
    '                    Me.dgvDetalle.Rows(Renglon).Cells(Me.TipoGanado.Index).Value = Detalle.TipoGanado.Descripcion
    '                    Me.dgvDetalle.Rows(Renglon).Cells(Me.Cabezas.Index).Value = Cabezas
    '                    Me.dgvDetalle.Rows(Renglon).Cells(Me.Kilos.Index).Value = Kilos
    '                    'Me.dgvDetalle.Rows(Renglon).Cells(Me.Importe.Index).Value = ImporteVenta
    '                    'Me.dgvDetalle.Rows(Renglon).Cells(Me.i.Index).Value = Detalle.TipoGanado.Descripcion
