...
Public Class FrmReporteDecomisos
    Dim Introductores As New CN.ClientesIntroductoresColeccion
    Private Sub FrmReporteDecomisos_Load(ByVal sender As System.Object, ByVal e As System.EventArgs) Handles MyBase.Load
        Try
    ...
    Private Sub MEAToolBar1_ClickImprimir(ByVal sender As Object, ByVal e As System.Windows.Forms.ToolBarButtonClickEventArgs, ByRef Cancelar As Boolean) Handles MEAToolBar1.ClickImprimir
        Try
            Dim Vista As New TC.VwReporteDecomisosTypedView
            Dim Filtro As New OC.PredicateExpression

            Filtro.Add(HC.VwReporteDecomisosFields.Estatus = 1)

            If Not Me.txtFolioSacrificio.Text = "" Then
                Filtro.Add(HC.VwReporteDecomisosFields.FolioSacrificio = Me.txtFolioSacrificio.Text)
            End If

            If Me.cmbIntroductor.SelectedIndex > -1 Then
                Filtro.Add(HC.VwReporteDecomisosFields.IdCodCliente = Me.cmbIntroductor.SelectedValue)
            End If

            If Me.txtFolioSacrificio.Text = "" Then
                Filtro.Add(New OC.FieldBetweenPredicate(HC.VwReporteDecomisosFields.FechaSacrificio, Me.dtFechaInicial.Value.ToShortDateString, _
               Me.DtFechaFinal.Value.AddDays(1).ToShortDateString))
            End If

            Vista.Fill(0, Nothing, False, Filtro)
            If Vista.Count > 0 Then
                Dim Reporte As New RptDecomisos
                ...
                Reporte.SetParameterValue(2, "Producción/Decomisos")

                Dim Ver As New CN.PreVisualizarForm
                Ver.Reporte = Reporte
                Ver.Show()
            Else
                MsgBox("No se encontrarón decomisos con la información especificada", MsgBoxStyle.Exclamation, "Aviso")
            End If
        Catch ex As Exception
            MsgBox(ex.Message, MsgBoxStyle.Critical, "Error")
        End Try
