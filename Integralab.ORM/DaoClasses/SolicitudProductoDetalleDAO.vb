...
Namespace Integralab.ORM.DaoClasses
	...
	''' <summary>
	''' General DAO class for the SolicitudProductoDetalle Entity. It will perform database oriented actions for
	''' a entity of type 'SolicitudProductoDetalleEntity'. This DAO works on an EntityFields object. 
	''' </summary>
	Public Class SolicitudProductoDetalleDAO 
		Inherits DaoBase
		
		''' <summary>CTor</summary>
		Public Sub New()
			MyBase.New(InheritanceInfoProviderSingleton.GetInstance(), New DynamicQueryEngine(), InheritanceHierarchyType.None, "SolicitudProductoDetalleEntity", New SolicitudProductoDetalleEntityFactory())
		End Sub
		...
		''' <summary>
		''' Retrieves in the calling SolicitudProductoDetalleCollection object all SolicitudProductoDetalleEntity objects which have data in common
		...
		Public Overloads Function GetMulti(containingTransaction As ITransaction, collectionToFill As IEntityCollection, maxNumberOfItemsToReturn As Long, sortClauses As ISortExpression, entityFactoryToUse As IEntityFactory, Filter As IPredicateExpression, productoInstance As IEntity, solicitudProductoInstance As IEntity, pageNumber As Integer, pageSize As Integer) As Boolean
			MyBase.EntityFactoryToUse = entityFactoryToUse
			Dim fieldsToReturn  As IEntityFields = EntityFieldsFactory.CreateEntityFieldsObject(Integralab.ORM.EntityType.SolicitudProductoDetalleEntity)
			Dim selectFilter As IPredicateExpression= CreateFilterUsingForeignKeys(productoInstance, solicitudProductoInstance, fieldsToReturn)
		...
		''' <summary>
		''' Retrieves entities of the type 'SolicitudProductoDetalleEntity' in a datatable which match the specified filter. It will always create a new connection to the database.
		''' </summary>
		''' <param name="maxNumberOfItemsToReturn"> The maximum number of items to return with this retrieval query. 
		''' If the used Dynamic Query Engine supports it, 'TOP' is used to limit the amount of rows to return. When set to 0, no limitations are specified.</param>
		''' <param name="sortClauses">The order by specifications for the sorting of the resultset. When not specified, no sorting is applied.</param>
		...
		''' <summary>
		''' Retrieves in the calling SolicitudProductoDetalleCollection object all SolicitudProductoDetalleEntity objects
		...
		Public Function GetMultiUsingProductoCollectionViaMcomCotizacionCompraProveedorDetalle(containingTransaction As ITransaction, collectionToFill As IEntityCollection, maxNumberOfItemsToReturn As Long, sortClauses As ISortExpression, entityFactoryToUse As IEntityFactory, productoInstance As IEntity, pageNumber As Integer, pageSize As Integer) As Boolean
			...
			relations.Add(SolicitudProductoDetalleEntity.Relations.McomCotizacionCompraProveedorDetalleEntityUsingIdSolicitudIdProducto, "McomCotizacionCompraProveedorDetalle_")
...
selectFilter.Add(New FieldCompareValuePredicate(mcomCotizacionCompraProveedorInstance.Fields(CInt(McomCotizacionCompraProveedorFieldIndex.IdProveedor)), ComparisonOperator.Equal))
		...
		''' <summary>
		''' Deletes from the persistent storage all 'SolicitudProductoDetalle' entities which have data in common
		''' with the specified related Entities. If one is omitted, that entity is not used as a filter. 
		''' </summary>
		''' <param name="containingTransaction">A containing transaction, if caller is added to a transaction, or Nothing if not.</param>
		''' <param name="productoInstance">ProductoEntity instance to use as a filter for the SolicitudProductoDetalleEntity objects to delete</param>
		''' <param name="solicitudProductoInstance">SolicitudProductoEntity instance to use as a filter for the SolicitudProductoDetalleEntity objects to delete</param>
		''' <returns>Amount of entities affected, if the used persistent storage has rowcounting enabled.</returns>
		Public Overloads Function DeleteMulti(containingTransaction As ITransaction, productoInstance As IEntity, solicitudProductoInstance As IEntity) As Integer
			Dim fields As IEntityFields = EntityFieldsFactory.CreateEntityFieldsObject(Integralab.ORM.EntityType.SolicitudProductoDetalleEntity)
			Dim deleteFilter As IPredicateExpression = CreateFilterUsingForeignKeys(productoInstance, solicitudProductoInstance, fields)
		...
		''' <param name="containingTransaction">A containing transaction, if caller is added to a transaction, or Nothing if not.</param>
		''' <param name="productoInstance">ProductoEntity instance to use as a filter for the SolicitudProductoDetalleEntity objects to update</param>
		''' <param name="solicitudProductoInstance">SolicitudProductoEntity instance to use as a filter for the SolicitudProductoDetalleEntity objects to update</param>
		''' <returns>Amount of entities affected, if the used persistent storage has rowcounting enabled.</returns>
		Public Overloads Function UpdateMulti(entityWithNewValues As IEntity, containingTransaction As ITransaction, productoInstance As IEntity, solicitudProductoInstance As IEntity) As Integer
			Dim fields As IEntityFields = EntityFieldsFactory.CreateEntityFieldsObject(Integralab.ORM.EntityType.SolicitudProductoDetalleEntity)
			Dim updateFilter As IPredicateExpression = CreateFilterUsingForeignKeys(productoInstance, solicitudProductoInstance, fields)
		...
		''' </summary>
		''' <param name="productoInstance">ProductoEntity instance to use as a filter for the SolicitudProductoDetalleEntity objects</param>
		''' <param name="solicitudProductoInstance">SolicitudProductoEntity instance to use as a filter for the SolicitudProductoDetalleEntity objects</param>
		''' <param name="fieldsToReturn">IEntityFields implementation which forms the definition of the fieldset of the target entity.</param>
		''' <returns>A ready to use PredicateExpression based on the passed in foreign key value holders.</returns>
		Private Function CreateFilterUsingForeignKeys(productoInstance As IEntity, solicitudProductoInstance As IEntity, fieldsToReturn As IEntityFields) As IPredicateExpression
			Dim selectFilter As IPredicateExpression = New PredicateExpression()
			
			If Not productoInstance Is Nothing Then
				selectFilter.Add(New FieldCompareValuePredicate(fieldsToReturn(CInt(SolicitudProductoDetalleFieldIndex.IdProducto)), ComparisonOperator.Equal, CType(productoInstance, ProductoEntity).Codigo))
			End If
			If Not solicitudProductoInstance Is Nothing Then
				selectFilter.Add(New FieldCompareValuePredicate(fieldsToReturn(CInt(SolicitudProductoDetalleFieldIndex.IdSolicitud)), ComparisonOperator.Equal, CType(solicitudProductoInstance, SolicitudProductoEntity).IdSolicitud))
			End If
			Return selectFilter
		End Function
