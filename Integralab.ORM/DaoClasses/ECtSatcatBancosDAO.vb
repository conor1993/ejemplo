...
Namespace Integralab.ORM.DaoClasses
	...
	''' <summary>
	''' General DAO class for the ECtSatcatBancos Entity. It will perform database oriented actions for
	''' a entity of type 'ECtSatcatBancosEntity'. This DAO works on an EntityFields object. 
	''' </summary>
	Public Class ECtSatcatBancosDAO 
		Inherits DaoBase
		
		''' <summary>CTor</summary>
		Public Sub New()
			MyBase.New(InheritanceInfoProviderSingleton.GetInstance(), New DynamicQueryEngine(), InheritanceHierarchyType.None, "ECtSatcatBancosEntity", New ECtSatcatBancosEntityFactory())
		End Sub
		...
		''' <summary>
		''' Retrieves entities of the type 'ECtSatcatBancosEntity' in a datatable which match the specified filter. It will always create a new connection to the database.
		''' </summary>
		...
		Public Overridable Function GetMultiAsDataTable(maxNumberOfItemsToReturn As Long, sortClauses As ISortExpression, selectFilter As IPredicate, relations As IRelationCollection, pageNumber As Integer, pageSize As Integer) As  DataTable
			Dim fieldsToReturn As IEntityFields = EntityFieldsFactory.CreateEntityFieldsObject(Integralab.ORM.EntityType.ECtSatcatBancosEntity)
