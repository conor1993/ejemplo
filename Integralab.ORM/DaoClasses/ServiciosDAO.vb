...
Namespace Integralab.ORM.DaoClasses
	...
	''' <summary>
	''' General DAO class for the Servicios Entity. It will perform database oriented actions for
	''' a entity of type 'ServiciosEntity'. This DAO works on an EntityFields object. 
	''' </summary>
	Public Class ServiciosDAO 
		...
		Public Sub New()
			MyBase.New(InheritanceInfoProviderSingleton.GetInstance(), New DynamicQueryEngine(), InheritanceHierarchyType.None, "ServiciosEntity", New ServiciosEntityFactory())
		End Sub
		...
		''' <summary>
		''' Retrieves entities of the type 'ServiciosEntity' in a datatable which match the specified filter. It will always create a new connection to the database.
		''' </summary>
		...
		Public Overridable Function GetMultiAsDataTable(maxNumberOfItemsToReturn As Long, sortClauses As ISortExpression, selectFilter As IPredicate, relations As IRelationCollection, pageNumber As Integer, pageSize As Integer) As  DataTable
			Dim fieldsToReturn As IEntityFields = EntityFieldsFactory.CreateEntityFieldsObject(Integralab.ORM.EntityType.ServiciosEntity)
		...
		''' <summary>
		''' Retrieves in the calling ServiciosCollection object all ServiciosEntity objects
		...
		Public Function GetMultiUsingMfacEmbarquesCabCollectionViaServiciosXembarque(containingTransaction As ITransaction, collectionToFill As IEntityCollection, maxNumberOfItemsToReturn As Long, sortClauses As ISortExpression, entityFactoryToUse As IEntityFactory, mfacEmbarquesCabInstance As IEntity, pageNumber As Integer, pageSize As Integer) As Boolean
			Dim fieldsToReturn As IEntityFields = EntityFieldsFactory.CreateEntityFieldsObject(Integralab.ORM.EntityType.ServiciosEntity)
			Dim relations As New RelationCollection()
			relations.Add(ServiciosEntity.Relations.ServiciosXembarqueEntityUsingIdServicio, "ServiciosXembarque_")
		...
		''' <returns>true if succeeded, false otherwise</returns>
		Public Function GetMultiUsingMfacEmbarquesCabCollectionViaServiciosXembarque(containingTransaction As ITransaction, collectionToFill As IEntityCollection, maxNumberOfItemsToReturn As Long, sortClauses As ISortExpression, entityFactoryToUse As IEntityFactory, mfacEmbarquesCabInstance As IEntity, prefetchPathToUse As IPrefetchPath) As Boolean
			Dim fieldsToReturn As IEntityFields = EntityFieldsFactory.CreateEntityFieldsObject(Integralab.ORM.EntityType.ServiciosEntity)
			Dim relations As  New RelationCollection()
			relations.Add(ServiciosEntity.Relations.ServiciosXembarqueEntityUsingIdServicio, "ServiciosXembarque_")
			relations.Add(ServiciosXembarqueEntity.Relations.MfacEmbarquesCabEntityUsingFoloEmbarque, "ServiciosXembarque_", String.Empty, JoinHint.None)
