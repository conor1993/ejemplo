...
Namespace Integralab.ORM.DaoClasses
	...
	''' <summary>
	''' General DAO class for the ReferenciaProductoDerivado Entity. It will perform database oriented actions for
	''' a entity of type 'ReferenciaProductoDerivadoEntity'. This DAO works on an EntityFields object. 
	''' </summary>
	Public Class ReferenciaProductoDerivadoDAO 
		Inherits DaoBase
		
		''' <summary>CTor</summary>
		Public Sub New()
			MyBase.New(InheritanceInfoProviderSingleton.GetInstance(), New DynamicQueryEngine(), InheritanceHierarchyType.None, "ReferenciaProductoDerivadoEntity", New ReferenciaProductoDerivadoEntityFactory())
		End Sub
		...
		''' <summary>
		''' Retrieves in the calling ReferenciaProductoDerivadoCollection object all ReferenciaProductoDerivadoEntity objects which have data in common
		...
		Public Overloads Function GetMulti(containingTransaction As ITransaction, collectionToFill As IEntityCollection, maxNumberOfItemsToReturn As Long, sortClauses As ISortExpression, entityFactoryToUse As IEntityFactory, Filter As IPredicateExpression, msccatProductosInstance As IEntity, pageNumber As Integer, pageSize As Integer) As Boolean
			...
			Dim fieldsToReturn  As IEntityFields = EntityFieldsFactory.CreateEntityFieldsObject(Integralab.ORM.EntityType.ReferenciaProductoDerivadoEntity)
		...
		''' <summary>
		''' Retrieves entities of the type 'ReferenciaProductoDerivadoEntity' in a datatable which match the specified filter. It will always create a new connection to the database.
		''' </summary>
		''' <param name="maxNumberOfItemsToReturn"> The maximum number of items to return with this retrieval query. 
		''' If the used Dynamic Query Engine supports it, 'TOP' is used to limit the amount of rows to return. When set to 0, no limitations are specified.</param>
		''' <param name="sortClauses">The order by specifications for the sorting of the resultset. When not specified, no sorting is applied.</param>
		...
		''' <summary>
		''' Deletes from the persistent storage all 'ReferenciaProductoDerivado' entities which have data in common
		''' with the specified related Entities. If one is omitted, that entity is not used as a filter. 
		''' </summary>
		''' <param name="containingTransaction">A containing transaction, if caller is added to a transaction, or Nothing if not.</param>
		''' <param name="msccatProductosInstance">MsccatProductosEntity instance to use as a filter for the ReferenciaProductoDerivadoEntity objects to delete</param>
		...
		Public Overloads Function DeleteMulti(containingTransaction As ITransaction, msccatProductosInstance As IEntity) As Integer
			Dim fields As IEntityFields = EntityFieldsFactory.CreateEntityFieldsObject(Integralab.ORM.EntityType.ReferenciaProductoDerivadoEntity)
		...
		''' <param name="containingTransaction">A containing transaction, if caller is added to a transaction, or Nothing if not.</param>
		''' <param name="msccatProductosInstance">MsccatProductosEntity instance to use as a filter for the ReferenciaProductoDerivadoEntity objects to update</param>
		''' <returns>Amount of entities affected, if the used persistent storage has rowcounting enabled.</returns>
		...
		''' </summary>
		''' <param name="msccatProductosInstance">MsccatProductosEntity instance to use as a filter for the ReferenciaProductoDerivadoEntity objects</param>
		...
		Private Function CreateFilterUsingForeignKeys(msccatProductosInstance As IEntity, fieldsToReturn As IEntityFields) As IPredicateExpression
			...
			If Not msccatProductosInstance Is Nothing Then
				selectFilter.Add(New FieldCompareValuePredicate(fieldsToReturn(CInt(ReferenciaProductoDerivadoFieldIndex.IdProducto)), ComparisonOperator.Equal, CType(msccatProductosInstance, MsccatProductosEntity).IdProducto))
			End If
			Return selectFilter
		End Function
