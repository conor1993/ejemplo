...
Namespace Integralab.ORM.DaoClasses
	...
	''' <summary>
	''' General DAO class for the MscregistroSacrificioCab Entity. It will perform database oriented actions for
	''' a entity of type 'MscregistroSacrificioCabEntity'. This DAO works on an EntityFields object. 
	''' </summary>
	Public Class MscregistroSacrificioCabDAO 
		Inherits DaoBase
		
		''' <summary>CTor</summary>
		Public Sub New()
			MyBase.New(InheritanceInfoProviderSingleton.GetInstance(), New DynamicQueryEngine(), InheritanceHierarchyType.None, "MscregistroSacrificioCabEntity", New MscregistroSacrificioCabEntityFactory())
		End Sub
		...
		''' <summary>
		''' Retrieves entities of the type 'MscregistroSacrificioCabEntity' in a datatable which match the specified filter. It will always create a new connection to the database.
		''' </summary>
		...
		Public Overridable Function GetMultiAsDataTable(maxNumberOfItemsToReturn As Long, sortClauses As ISortExpression, selectFilter As IPredicate, relations As IRelationCollection, pageNumber As Integer, pageSize As Integer) As  DataTable
			Dim fieldsToReturn As IEntityFields = EntityFieldsFactory.CreateEntityFieldsObject(Integralab.ORM.EntityType.MscregistroSacrificioCabEntity)
		...
		''' <summary>
		''' Retrieves in the calling MscregistroSacrificioCabCollection object all MscregistroSacrificioCabEntity objects
		...
		Public Function GetMultiUsingRecepcionGanadoCollectionViaUsrProdRecepcionGanadoDet(containingTransaction As ITransaction, collectionToFill As IEntityCollection, maxNumberOfItemsToReturn As Long, sortClauses As ISortExpression, entityFactoryToUse As IEntityFactory, recepcionGanadoInstance As IEntity, pageNumber As Integer, pageSize As Integer) As Boolean
			Dim fieldsToReturn As IEntityFields = EntityFieldsFactory.CreateEntityFieldsObject(Integralab.ORM.EntityType.MscregistroSacrificioCabEntity)
			Dim relations As New RelationCollection()
			relations.Add(MscregistroSacrificioCabEntity.Relations.UsrProdRecepcionGanadoDetEntityUsingLoteSacrificio, "UsrProdRecepcionGanadoDet_")
