...
Namespace Integralab.ORM.DaoClasses
	...
	''' <summary>
	''' General DAO class for the CatLugCom Entity. It will perform database oriented actions for
	''' a entity of type 'CatLugComEntity'. This DAO works on an EntityFields object. 
	''' </summary>
	Public Class CatLugComDAO 
		Inherits DaoBase
		
		''' <summary>CTor</summary>
		Public Sub New()
			MyBase.New(InheritanceInfoProviderSingleton.GetInstance(), New DynamicQueryEngine(), InheritanceHierarchyType.None, "CatLugComEntity", New CatLugComEntityFactory())
		End Sub
		...
		''' <summary>
		''' Retrieves entities of the type 'CatLugComEntity' in a datatable which match the specified filter. It will always create a new connection to the database.
		''' </summary>
		...
		Public Overridable Function GetMultiAsDataTable(maxNumberOfItemsToReturn As Long, sortClauses As ISortExpression, selectFilter As IPredicate, relations As IRelationCollection, pageNumber As Integer, pageSize As Integer) As  DataTable
			Dim fieldsToReturn As IEntityFields = EntityFieldsFactory.CreateEntityFieldsObject(Integralab.ORM.EntityType.CatLugComEntity)
