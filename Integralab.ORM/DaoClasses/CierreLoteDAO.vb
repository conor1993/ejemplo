...
Namespace Integralab.ORM.DaoClasses
	...
	''' <summary>
	''' General DAO class for the CierreLote Entity. It will perform database oriented actions for
	''' a entity of type 'CierreLoteEntity'. This DAO works on an EntityFields object. 
	''' </summary>
	Public Class CierreLoteDAO 
		Inherits DaoBase
		
		''' <summary>CTor</summary>
		Public Sub New()
			MyBase.New(InheritanceInfoProviderSingleton.GetInstance(), New DynamicQueryEngine(), InheritanceHierarchyType.None, "CierreLoteEntity", New CierreLoteEntityFactory())
		End Sub
		...
		''' <summary>
		''' Retrieves in the calling CierreLoteCollection object all CierreLoteEntity objects which have data in common
		...
		Public Overloads Function GetMulti(containingTransaction As ITransaction, collectionToFill As IEntityCollection, maxNumberOfItemsToReturn As Long, sortClauses As ISortExpression, entityFactoryToUse As IEntityFactory, Filter As IPredicateExpression, mcecatLotesCabInstance As IEntity, pageNumber As Integer, pageSize As Integer) As Boolean
			MyBase.EntityFactoryToUse = entityFactoryToUse
			Dim fieldsToReturn  As IEntityFields = EntityFieldsFactory.CreateEntityFieldsObject(Integralab.ORM.EntityType.CierreLoteEntity)
			Dim selectFilter As IPredicateExpression= CreateFilterUsingForeignKeys(mcecatLotesCabInstance, fieldsToReturn)
		...
		''' <summary>
		''' Retrieves entities of the type 'CierreLoteEntity' in a datatable which match the specified filter. It will always create a new connection to the database.
		''' </summary>
		''' <param name="maxNumberOfItemsToReturn"> The maximum number of items to return with this retrieval query. 
		''' If the used Dynamic Query Engine supports it, 'TOP' is used to limit the amount of rows to return. When set to 0, no limitations are specified.</param>
		''' <param name="sortClauses">The order by specifications for the sorting of the resultset. When not specified, no sorting is applied.</param>
		...
		''' <summary>
		''' Retrieves in the calling CierreLoteCollection object all CierreLoteEntity objects
		...
		''' <summary>
		''' Deletes from the persistent storage all 'CierreLote' entities which have data in common
		''' with the specified related Entities. If one is omitted, that entity is not used as a filter. 
		''' </summary>
		''' <param name="containingTransaction">A containing transaction, if caller is added to a transaction, or Nothing if not.</param>
		''' <param name="mcecatLotesCabInstance">McecatLotesCabEntity instance to use as a filter for the CierreLoteEntity objects to delete</param>
		''' <returns>Amount of entities affected, if the used persistent storage has rowcounting enabled.</returns>
		Public Overloads Function DeleteMulti(containingTransaction As ITransaction, mcecatLotesCabInstance As IEntity) As Integer
			Dim fields As IEntityFields = EntityFieldsFactory.CreateEntityFieldsObject(Integralab.ORM.EntityType.CierreLoteEntity)
			Dim deleteFilter As IPredicateExpression = CreateFilterUsingForeignKeys(mcecatLotesCabInstance, fields)
		...
		''' <param name="containingTransaction">A containing transaction, if caller is added to a transaction, or Nothing if not.</param>
		''' <param name="mcecatLotesCabInstance">McecatLotesCabEntity instance to use as a filter for the CierreLoteEntity objects to update</param>
		''' <returns>Amount of entities affected, if the used persistent storage has rowcounting enabled.</returns>
		Public Overloads Function UpdateMulti(entityWithNewValues As IEntity, containingTransaction As ITransaction, mcecatLotesCabInstance As IEntity) As Integer
			Dim fields As IEntityFields = EntityFieldsFactory.CreateEntityFieldsObject(Integralab.ORM.EntityType.CierreLoteEntity)
			Dim updateFilter As IPredicateExpression = CreateFilterUsingForeignKeys(mcecatLotesCabInstance, fields)
		...
		''' </summary>
		''' <param name="mcecatLotesCabInstance">McecatLotesCabEntity instance to use as a filter for the CierreLoteEntity objects</param>
		''' <param name="fieldsToReturn">IEntityFields implementation which forms the definition of the fieldset of the target entity.</param>
		''' <returns>A ready to use PredicateExpression based on the passed in foreign key value holders.</returns>
		Private Function CreateFilterUsingForeignKeys(mcecatLotesCabInstance As IEntity, fieldsToReturn As IEntityFields) As IPredicateExpression
			Dim selectFilter As IPredicateExpression = New PredicateExpression()
			
			If Not mcecatLotesCabInstance Is Nothing Then
				selectFilter.Add(New FieldCompareValuePredicate(fieldsToReturn(CInt(CierreLoteFieldIndex.IdLote)), ComparisonOperator.Equal, CType(mcecatLotesCabInstance, McecatLotesCabEntity).IdLote))
			End If
			Return selectFilter
		End Function
