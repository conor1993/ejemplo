...
Namespace Integralab.ORM.DaoClasses
	...
	''' <summary>
	''' General DAO class for the SolicitudProducto Entity. It will perform database oriented actions for
	''' a entity of type 'SolicitudProductoEntity'. This DAO works on an EntityFields object. 
	''' </summary>
	Public Class SolicitudProductoDAO 
		...
		Public Sub New()
			MyBase.New(InheritanceInfoProviderSingleton.GetInstance(), New DynamicQueryEngine(), InheritanceHierarchyType.None, "SolicitudProductoEntity", New SolicitudProductoEntityFactory())
		End Sub
		...
		''' <summary>
		''' Retrieves in the calling SolicitudProductoCollection object all SolicitudProductoEntity objects which have data in common
		...
		Public Overloads Function GetMulti(containingTransaction As ITransaction, collectionToFill As IEntityCollection, maxNumberOfItemsToReturn As Long, sortClauses As ISortExpression, entityFactoryToUse As IEntityFactory, Filter As IPredicateExpression, sucursalInstance As IEntity, pageNumber As Integer, pageSize As Integer) As Boolean
			...
			Dim fieldsToReturn  As IEntityFields = EntityFieldsFactory.CreateEntityFieldsObject(Integralab.ORM.EntityType.SolicitudProductoEntity)
			Dim selectFilter As IPredicateExpression= CreateFilterUsingForeignKeys(sucursalInstance, fieldsToReturn)
		...
		''' <summary>
		''' Retrieves entities of the type 'SolicitudProductoEntity' in a datatable which match the specified filter. It will always create a new connection to the database.
		''' </summary>
		''' <param name="maxNumberOfItemsToReturn"> The maximum number of items to return with this retrieval query. 
		''' If the used Dynamic Query Engine supports it, 'TOP' is used to limit the amount of rows to return. When set to 0, no limitations are specified.</param>
		''' <param name="sortClauses">The order by specifications for the sorting of the resultset. When not specified, no sorting is applied.</param>
		...
		''' <summary>
		''' Retrieves in the calling SolicitudProductoCollection object all SolicitudProductoEntity objects
		...
		Public Function GetMultiUsingProductoCollectionViaOrdenCompraDetalle(containingTransaction As ITransaction, collectionToFill As IEntityCollection, maxNumberOfItemsToReturn As Long, sortClauses As ISortExpression, entityFactoryToUse As IEntityFactory, productoInstance As IEntity, pageNumber As Integer, pageSize As Integer) As Boolean
			...
			relations.Add(SolicitudProductoEntity.Relations.OrdenCompraDetalleEntityUsingIdSolicitud, "OrdenCompraDetalle_")
		...
		''' <param name="pageSize">The page size of the page to retrieve.</param>
		''' <returns>true if succeeded, false otherwise</returns>
		Public Function GetMultiUsingMcomCotizacionCompraCollectionViaOrdenCompraDetalle(containingTransaction As ITransaction, collectionToFill As IEntityCollection, maxNumberOfItemsToReturn As Long, sortClauses As ISortExpression, entityFactoryToUse As IEntityFactory, mcomCotizacionCompraInstance As IEntity, pageNumber As Integer, pageSize As Integer) As Boolean
			Dim fieldsToReturn As IEntityFields = EntityFieldsFactory.CreateEntityFieldsObject(Integralab.ORM.EntityType.SolicitudProductoEntity)
			Dim relations As New RelationCollection()
			relations.Add(SolicitudProductoEntity.Relations.OrdenCompraDetalleEntityUsingIdSolicitud, "OrdenCompraDetalle_")
		...
		''' <param name="prefetchPathToUse">the PrefetchPath which defines the graph of objects to fetch.</param>
		''' <returns>true if succeeded, false otherwise</returns>
		Public Function GetMultiUsingMcomCotizacionCompraCollectionViaOrdenCompraDetalle(containingTransaction As ITransaction, collectionToFill As IEntityCollection, maxNumberOfItemsToReturn As Long, sortClauses As ISortExpression, entityFactoryToUse As IEntityFactory, mcomCotizacionCompraInstance As IEntity, prefetchPathToUse As IPrefetchPath) As Boolean
			Dim fieldsToReturn As IEntityFields = EntityFieldsFactory.CreateEntityFieldsObject(Integralab.ORM.EntityType.SolicitudProductoEntity)
			Dim relations As  New RelationCollection()
			relations.Add(SolicitudProductoEntity.Relations.OrdenCompraDetalleEntityUsingIdSolicitud, "OrdenCompraDetalle_")
			relations.Add(OrdenCompraDetalleEntity.Relations.McomCotizacionCompraEntityUsingIdCotizacion, "OrdenCompraDetalle_", String.Empty, JoinHint.None)
		...
		''' <summary>
		''' Deletes from the persistent storage all 'SolicitudProducto' entities which have data in common
		''' with the specified related Entities. If one is omitted, that entity is not used as a filter. 
		''' </summary>
		''' <param name="containingTransaction">A containing transaction, if caller is added to a transaction, or Nothing if not.</param>
		''' <param name="sucursalInstance">SucursalEntity instance to use as a filter for the SolicitudProductoEntity objects to delete</param>
		''' <returns>Amount of entities affected, if the used persistent storage has rowcounting enabled.</returns>
		Public Overloads Function DeleteMulti(containingTransaction As ITransaction, sucursalInstance As IEntity) As Integer
			Dim fields As IEntityFields = EntityFieldsFactory.CreateEntityFieldsObject(Integralab.ORM.EntityType.SolicitudProductoEntity)
			Dim deleteFilter As IPredicateExpression = CreateFilterUsingForeignKeys(sucursalInstance, fields)
		...
		''' <param name="containingTransaction">A containing transaction, if caller is added to a transaction, or Nothing if not.</param>
		''' <param name="sucursalInstance">SucursalEntity instance to use as a filter for the SolicitudProductoEntity objects to update</param>
		''' <returns>Amount of entities affected, if the used persistent storage has rowcounting enabled.</returns>
		Public Overloads Function UpdateMulti(entityWithNewValues As IEntity, containingTransaction As ITransaction, sucursalInstance As IEntity) As Integer
			Dim fields As IEntityFields = EntityFieldsFactory.CreateEntityFieldsObject(Integralab.ORM.EntityType.SolicitudProductoEntity)
			Dim updateFilter As IPredicateExpression = CreateFilterUsingForeignKeys(sucursalInstance, fields)
		...
		''' </summary>
		''' <param name="sucursalInstance">SucursalEntity instance to use as a filter for the SolicitudProductoEntity objects</param>
		''' <param name="fieldsToReturn">IEntityFields implementation which forms the definition of the fieldset of the target entity.</param>
		''' <returns>A ready to use PredicateExpression based on the passed in foreign key value holders.</returns>
		Private Function CreateFilterUsingForeignKeys(sucursalInstance As IEntity, fieldsToReturn As IEntityFields) As IPredicateExpression
			Dim selectFilter As IPredicateExpression = New PredicateExpression()
			
			If Not sucursalInstance Is Nothing Then
				selectFilter.Add(New FieldCompareValuePredicate(fieldsToReturn(CInt(SolicitudProductoFieldIndex.IdSucursal)), ComparisonOperator.Equal, CType(sucursalInstance, SucursalEntity).Codigo))
			End If
			Return selectFilter
		End Function
