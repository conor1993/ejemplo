...
Namespace Integralab.ORM.DaoClasses
	...
	''' <summary>
	''' General DAO class for the CatDeptos Entity. It will perform database oriented actions for
	''' a entity of type 'CatDeptosEntity'. This DAO works on an EntityFields object. 
	''' </summary>
	Public Class CatDeptosDAO 
		Inherits DaoBase
		
		''' <summary>CTor</summary>
		Public Sub New()
			MyBase.New(InheritanceInfoProviderSingleton.GetInstance(), New DynamicQueryEngine(), InheritanceHierarchyType.None, "CatDeptosEntity", New CatDeptosEntityFactory())
		End Sub
		...
		''' <summary>
		''' Retrieves entities of the type 'CatDeptosEntity' in a datatable which match the specified filter. It will always create a new connection to the database.
		''' </summary>
		...
		Public Overridable Function GetMultiAsDataTable(maxNumberOfItemsToReturn As Long, sortClauses As ISortExpression, selectFilter As IPredicate, relations As IRelationCollection, pageNumber As Integer, pageSize As Integer) As  DataTable
			Dim fieldsToReturn As IEntityFields = EntityFieldsFactory.CreateEntityFieldsObject(Integralab.ORM.EntityType.CatDeptosEntity)
		...
		''' <summary>
		''' Retrieves in the calling CatDeptosCollection object all CatDeptosEntity objects
		...
		Public Function GetMultiUsingMetodoCabCollectionViaMetodoDet(containingTransaction As ITransaction, collectionToFill As IEntityCollection, maxNumberOfItemsToReturn As Long, sortClauses As ISortExpression, entityFactoryToUse As IEntityFactory, metodoCabInstance As IEntity, pageNumber As Integer, pageSize As Integer) As Boolean
			Dim fieldsToReturn As IEntityFields = EntityFieldsFactory.CreateEntityFieldsObject(Integralab.ORM.EntityType.CatDeptosEntity)
			Dim relations As New RelationCollection()
			relations.Add(CatDeptosEntity.Relations.MetodoDetEntityUsingCodCentroCosto, "MetodoDet_")
		...
		''' <returns>true if succeeded, false otherwise</returns>
		Public Function GetMultiUsingMetodoCabCollectionViaMetodoDet(containingTransaction As ITransaction, collectionToFill As IEntityCollection, maxNumberOfItemsToReturn As Long, sortClauses As ISortExpression, entityFactoryToUse As IEntityFactory, metodoCabInstance As IEntity, prefetchPathToUse As IPrefetchPath) As Boolean
			Dim fieldsToReturn As IEntityFields = EntityFieldsFactory.CreateEntityFieldsObject(Integralab.ORM.EntityType.CatDeptosEntity)
			Dim relations As  New RelationCollection()
			relations.Add(CatDeptosEntity.Relations.MetodoDetEntityUsingCodCentroCosto, "MetodoDet_")
			relations.Add(MetodoDetEntity.Relations.MetodoCabEntityUsingCodMetodo, "MetodoDet_", String.Empty, JoinHint.None)
		...
		Public Function GetMultiUsingComicionistaCollectionViaMfacCatClientes(containingTransaction As ITransaction, collectionToFill As IEntityCollection, maxNumberOfItemsToReturn As Long, sortClauses As ISortExpression, entityFactoryToUse As IEntityFactory, comicionistaInstance As IEntity, pageNumber As Integer, pageSize As Integer) As Boolean
			Dim fieldsToReturn As IEntityFields = EntityFieldsFactory.CreateEntityFieldsObject(Integralab.ORM.EntityType.CatDeptosEntity)
			Dim relations As New RelationCollection()
			relations.Add(CatDeptosEntity.Relations.MfacCatClientesEntityUsingIdDepartamento, "MfacCatClientes_")
		...
		''' <summary>
		''' Retrieves in the calling CatDeptosCollection object all CatDeptosEntity objects
		''' which are related via a relation of type 'm:n' with the passed in NomCatPuestosEntity. 
		''' </summary>
		''' <param name="containingTransaction">A containing transaction, if caller is added to a transaction, or Nothing if not.</param>
		''' <param name="collectionToFill">Collection to fill with the entity objects retrieved</param>
		''' <param name="maxNumberOfItemsToReturn"> The maximum number of items to return with this retrieval query. 
		...
		''' <param name="pageSize">The page size of the page to retrieve.</param>
		''' <returns>true if succeeded, false otherwise</returns>
		Public Function GetMultiUsingNomCatPuestosCollectionViaNomCatEmpleados(containingTransaction As ITransaction, collectionToFill As IEntityCollection, maxNumberOfItemsToReturn As Long, sortClauses As ISortExpression, entityFactoryToUse As IEntityFactory, nomCatPuestosInstance As IEntity, pageNumber As Integer, pageSize As Integer) As Boolean
			Dim fieldsToReturn As IEntityFields = EntityFieldsFactory.CreateEntityFieldsObject(Integralab.ORM.EntityType.CatDeptosEntity)
			Dim relations As New RelationCollection()
			relations.Add(CatDeptosEntity.Relations.NomCatEmpleadosEntityUsingIdDepartamento, "NomCatEmpleados_")
			...
			Dim selectFilter As IPredicateExpression = New PredicateExpression()
			selectFilter.Add(New FieldCompareValuePredicate(nomCatPuestosInstance.Fields(CInt(NomCatPuestosFieldIndex.Codigo)), ComparisonOperator.Equal))
			Return GetMulti(containingTransaction, collectionToFill, maxNumberOfItemsToReturn, sortClauses, entityFactoryToUse, selectFilter, relations, pageNumber, pageSize)
		End Function
		...
		''' <param name="prefetchPathToUse">the PrefetchPath which defines the graph of objects to fetch.</param>
		''' <returns>true if succeeded, false otherwise</returns>
		Public Function GetMultiUsingNomCatPuestosCollectionViaNomCatEmpleados(containingTransaction As ITransaction, collectionToFill As IEntityCollection, maxNumberOfItemsToReturn As Long, sortClauses As ISortExpression, entityFactoryToUse As IEntityFactory, nomCatPuestosInstance As IEntity, prefetchPathToUse As IPrefetchPath) As Boolean
			Dim fieldsToReturn As IEntityFields = EntityFieldsFactory.CreateEntityFieldsObject(Integralab.ORM.EntityType.CatDeptosEntity)
			Dim relations As  New RelationCollection()
			relations.Add(CatDeptosEntity.Relations.NomCatEmpleadosEntityUsingIdDepartamento, "NomCatEmpleados_")
			relations.Add(NomCatEmpleadosEntity.Relations.NomCatPuestosEntityUsingIdPuesto, "NomCatEmpleados_", String.Empty, JoinHint.None)
