...
Namespace Integralab.ORM.DaoClasses
	...
	''' <summary>
	''' General DAO class for the SalidaGanadoCab Entity. It will perform database oriented actions for
	''' a entity of type 'SalidaGanadoCabEntity'. This DAO works on an EntityFields object. 
	''' </summary>
	Public Class SalidaGanadoCabDAO 
		Inherits DaoBase
		
		''' <summary>CTor</summary>
		Public Sub New()
			MyBase.New(InheritanceInfoProviderSingleton.GetInstance(), New DynamicQueryEngine(), InheritanceHierarchyType.None, "SalidaGanadoCabEntity", New SalidaGanadoCabEntityFactory())
		End Sub
		...
		''' <summary>
		''' Retrieves in the calling SalidaGanadoCabCollection object all SalidaGanadoCabEntity objects which have data in common
		...
		Public Overloads Function GetMulti(containingTransaction As ITransaction, collectionToFill As IEntityCollection, maxNumberOfItemsToReturn As Long, sortClauses As ISortExpression, entityFactoryToUse As IEntityFactory, Filter As IPredicateExpression, cabMovGanInstance As IEntity, mcecatLotesCabInstance As IEntity, mcgcatCausasMuerteInstance As IEntity, catRastrosInstance As IEntity, mcecatCorralesCabInstance As IEntity, polizaInstance As IEntity, pageNumber As Integer, pageSize As Integer) As Boolean
			MyBase.EntityFactoryToUse = entityFactoryToUse
			Dim fieldsToReturn  As IEntityFields = EntityFieldsFactory.CreateEntityFieldsObject(Integralab.ORM.EntityType.SalidaGanadoCabEntity)
			Dim selectFilter As IPredicateExpression= CreateFilterUsingForeignKeys(cabMovGanInstance, mcecatLotesCabInstance, mcgcatCausasMuerteInstance, catRastrosInstance, mcecatCorralesCabInstance, polizaInstance, fieldsToReturn)
		...
		''' <summary>
		''' Retrieves entities of the type 'SalidaGanadoCabEntity' in a datatable which match the specified filter. It will always create a new connection to the database.
		''' </summary>
		''' <param name="maxNumberOfItemsToReturn"> The maximum number of items to return with this retrieval query. 
		''' If the used Dynamic Query Engine supports it, 'TOP' is used to limit the amount of rows to return. When set to 0, no limitations are specified.</param>
		''' <param name="sortClauses">The order by specifications for the sorting of the resultset. When not specified, no sorting is applied.</param>
		...
		''' <summary>
		''' Retrieves in the calling SalidaGanadoCabCollection object all SalidaGanadoCabEntity objects
		...
		Public Function GetMultiUsingPolizaCollectionViaCabFacturas(containingTransaction As ITransaction, collectionToFill As IEntityCollection, maxNumberOfItemsToReturn As Long, sortClauses As ISortExpression, entityFactoryToUse As IEntityFactory, polizaInstance As IEntity, pageNumber As Integer, pageSize As Integer) As Boolean
			Dim fieldsToReturn As IEntityFields = EntityFieldsFactory.CreateEntityFieldsObject(Integralab.ORM.EntityType.SalidaGanadoCabEntity)
			Dim relations As New RelationCollection()
			relations.Add(SalidaGanadoCabEntity.Relations.CabFacturasEntityUsingReferencia, "CabFacturas_")
		...
		''' <returns>true if succeeded, false otherwise</returns>
		Public Function GetMultiUsingPolizaCollectionViaCabFacturas(containingTransaction As ITransaction, collectionToFill As IEntityCollection, maxNumberOfItemsToReturn As Long, sortClauses As ISortExpression, entityFactoryToUse As IEntityFactory, polizaInstance As IEntity, prefetchPathToUse As IPrefetchPath) As Boolean
			Dim fieldsToReturn As IEntityFields = EntityFieldsFactory.CreateEntityFieldsObject(Integralab.ORM.EntityType.SalidaGanadoCabEntity)
			Dim relations As  New RelationCollection()
			relations.Add(SalidaGanadoCabEntity.Relations.CabFacturasEntityUsingReferencia, "CabFacturas_")
			relations.Add(CabFacturasEntity.Relations.PolizaEntityUsingFolPoliza, "CabFacturas_", String.Empty, JoinHint.None)
		...
		''' <param name="pageSize">The page size of the page to retrieve.</param>
		''' <returns>true if succeeded, false otherwise</returns>
		Public Function GetMultiUsingMcecatCorralesCabCollectionViaProrraRec(containingTransaction As ITransaction, collectionToFill As IEntityCollection, maxNumberOfItemsToReturn As Long, sortClauses As ISortExpression, entityFactoryToUse As IEntityFactory, mcecatCorralesCabInstance As IEntity, pageNumber As Integer, pageSize As Integer) As Boolean
			Dim fieldsToReturn As IEntityFields = EntityFieldsFactory.CreateEntityFieldsObject(Integralab.ORM.EntityType.SalidaGanadoCabEntity)
			Dim relations As New RelationCollection()
			relations.Add(SalidaGanadoCabEntity.Relations.ProrraRecEntityUsingIdSalidaGanadoCab, "ProrraRec_")
		...
		Public Function GetMultiUsingMcecatLotesCabCollectionViaSalidaGanadoDet(containingTransaction As ITransaction, collectionToFill As IEntityCollection, maxNumberOfItemsToReturn As Long, sortClauses As ISortExpression, entityFactoryToUse As IEntityFactory, mcecatLotesCabInstance As IEntity, pageNumber As Integer, pageSize As Integer) As Boolean
			Dim fieldsToReturn As IEntityFields = EntityFieldsFactory.CreateEntityFieldsObject(Integralab.ORM.EntityType.SalidaGanadoCabEntity)
			Dim relations As New RelationCollection()
			relations.Add(SalidaGanadoCabEntity.Relations.SalidaGanadoDetEntityUsingIdSalidaGanadoCab, "SalidaGanadoDet_")
		...
		''' <summary>
		''' Deletes from the persistent storage all 'SalidaGanadoCab' entities which have data in common
		''' with the specified related Entities. If one is omitted, that entity is not used as a filter. 
		''' </summary>
		''' <param name="containingTransaction">A containing transaction, if caller is added to a transaction, or Nothing if not.</param>
		''' <param name="cabMovGanInstance">CabMovGanEntity instance to use as a filter for the SalidaGanadoCabEntity objects to delete</param>
		''' <param name="mcecatLotesCabInstance">McecatLotesCabEntity instance to use as a filter for the SalidaGanadoCabEntity objects to delete</param>
		''' <param name="mcgcatCausasMuerteInstance">McgcatCausasMuerteEntity instance to use as a filter for the SalidaGanadoCabEntity objects to delete</param>
		''' <param name="catRastrosInstance">CatRastrosEntity instance to use as a filter for the SalidaGanadoCabEntity objects to delete</param>
		''' <param name="mcecatCorralesCabInstance">McecatCorralesCabEntity instance to use as a filter for the SalidaGanadoCabEntity objects to delete</param>
		''' <param name="polizaInstance">PolizaEntity instance to use as a filter for the SalidaGanadoCabEntity objects to delete</param>
		''' <returns>Amount of entities affected, if the used persistent storage has rowcounting enabled.</returns>
		Public Overloads Function DeleteMulti(containingTransaction As ITransaction, cabMovGanInstance As IEntity, mcecatLotesCabInstance As IEntity, mcgcatCausasMuerteInstance As IEntity, catRastrosInstance As IEntity, mcecatCorralesCabInstance As IEntity, polizaInstance As IEntity) As Integer
			Dim fields As IEntityFields = EntityFieldsFactory.CreateEntityFieldsObject(Integralab.ORM.EntityType.SalidaGanadoCabEntity)
			Dim deleteFilter As IPredicateExpression = CreateFilterUsingForeignKeys(cabMovGanInstance, mcecatLotesCabInstance, mcgcatCausasMuerteInstance, catRastrosInstance, mcecatCorralesCabInstance, polizaInstance, fields)
		...
		''' <param name="containingTransaction">A containing transaction, if caller is added to a transaction, or Nothing if not.</param>
		''' <param name="cabMovGanInstance">CabMovGanEntity instance to use as a filter for the SalidaGanadoCabEntity objects to update</param>
		''' <param name="mcecatLotesCabInstance">McecatLotesCabEntity instance to use as a filter for the SalidaGanadoCabEntity objects to update</param>
		''' <param name="mcgcatCausasMuerteInstance">McgcatCausasMuerteEntity instance to use as a filter for the SalidaGanadoCabEntity objects to update</param>
		''' <param name="catRastrosInstance">CatRastrosEntity instance to use as a filter for the SalidaGanadoCabEntity objects to update</param>
		''' <param name="mcecatCorralesCabInstance">McecatCorralesCabEntity instance to use as a filter for the SalidaGanadoCabEntity objects to update</param>
		''' <param name="polizaInstance">PolizaEntity instance to use as a filter for the SalidaGanadoCabEntity objects to update</param>
		''' <returns>Amount of entities affected, if the used persistent storage has rowcounting enabled.</returns>
		Public Overloads Function UpdateMulti(entityWithNewValues As IEntity, containingTransaction As ITransaction, cabMovGanInstance As IEntity, mcecatLotesCabInstance As IEntity, mcgcatCausasMuerteInstance As IEntity, catRastrosInstance As IEntity, mcecatCorralesCabInstance As IEntity, polizaInstance As IEntity) As Integer
			Dim fields As IEntityFields = EntityFieldsFactory.CreateEntityFieldsObject(Integralab.ORM.EntityType.SalidaGanadoCabEntity)
			Dim updateFilter As IPredicateExpression = CreateFilterUsingForeignKeys(cabMovGanInstance, mcecatLotesCabInstance, mcgcatCausasMuerteInstance, catRastrosInstance, mcecatCorralesCabInstance, polizaInstance, fields)
		...
		''' </summary>
		''' <param name="cabMovGanInstance">CabMovGanEntity instance to use as a filter for the SalidaGanadoCabEntity objects</param>
		''' <param name="mcecatLotesCabInstance">McecatLotesCabEntity instance to use as a filter for the SalidaGanadoCabEntity objects</param>
		''' <param name="mcgcatCausasMuerteInstance">McgcatCausasMuerteEntity instance to use as a filter for the SalidaGanadoCabEntity objects</param>
		''' <param name="catRastrosInstance">CatRastrosEntity instance to use as a filter for the SalidaGanadoCabEntity objects</param>
		''' <param name="mcecatCorralesCabInstance">McecatCorralesCabEntity instance to use as a filter for the SalidaGanadoCabEntity objects</param>
		''' <param name="polizaInstance">PolizaEntity instance to use as a filter for the SalidaGanadoCabEntity objects</param>
		''' <param name="fieldsToReturn">IEntityFields implementation which forms the definition of the fieldset of the target entity.</param>
		''' <returns>A ready to use PredicateExpression based on the passed in foreign key value holders.</returns>
		Private Function CreateFilterUsingForeignKeys(cabMovGanInstance As IEntity, mcecatLotesCabInstance As IEntity, mcgcatCausasMuerteInstance As IEntity, catRastrosInstance As IEntity, mcecatCorralesCabInstance As IEntity, polizaInstance As IEntity, fieldsToReturn As IEntityFields) As IPredicateExpression
			Dim selectFilter As IPredicateExpression = New PredicateExpression()
			
			If Not cabMovGanInstance Is Nothing Then
				selectFilter.Add(New FieldCompareValuePredicate(fieldsToReturn(CInt(SalidaGanadoCabFieldIndex.FolioMovimientoGanado)), ComparisonOperator.Equal, CType(cabMovGanInstance, CabMovGanEntity).FolMovGan))
			End If
			If Not mcecatLotesCabInstance Is Nothing Then
				selectFilter.Add(New FieldCompareValuePredicate(fieldsToReturn(CInt(SalidaGanadoCabFieldIndex.IdLoteDestino)), ComparisonOperator.Equal, CType(mcecatLotesCabInstance, McecatLotesCabEntity).IdLote))
			End If
			If Not mcgcatCausasMuerteInstance Is Nothing Then
				selectFilter.Add(New FieldCompareValuePredicate(fieldsToReturn(CInt(SalidaGanadoCabFieldIndex.IdCausaMuerte)), ComparisonOperator.Equal, CType(mcgcatCausasMuerteInstance, McgcatCausasMuerteEntity).IdCausaMuerte))
			End If
			If Not catRastrosInstance Is Nothing Then
				selectFilter.Add(New FieldCompareValuePredicate(fieldsToReturn(CInt(SalidaGanadoCabFieldIndex.IdRastro)), ComparisonOperator.Equal, CType(catRastrosInstance, CatRastrosEntity).IdRastro))
			End If
			If Not mcecatCorralesCabInstance Is Nothing Then
				selectFilter.Add(New FieldCompareValuePredicate(fieldsToReturn(CInt(SalidaGanadoCabFieldIndex.IdCorral)), ComparisonOperator.Equal, CType(mcecatCorralesCabInstance, McecatCorralesCabEntity).IdCorral))
			End If
			If Not polizaInstance Is Nothing Then
				selectFilter.Add(New FieldCompareValuePredicate(fieldsToReturn(CInt(SalidaGanadoCabFieldIndex.IdPoliza)), ComparisonOperator.Equal, CType(polizaInstance, PolizaEntity).Codigo))
			End If
			Return selectFilter
		End Function
