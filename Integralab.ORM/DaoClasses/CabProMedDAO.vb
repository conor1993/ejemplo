...
Namespace Integralab.ORM.DaoClasses
	...
	''' <summary>
	''' General DAO class for the CabProMed Entity. It will perform database oriented actions for
	''' a entity of type 'CabProMedEntity'. This DAO works on an EntityFields object. 
	''' </summary>
	Public Class CabProMedDAO 
		Inherits DaoBase
		
		''' <summary>CTor</summary>
		Public Sub New()
			MyBase.New(InheritanceInfoProviderSingleton.GetInstance(), New DynamicQueryEngine(), InheritanceHierarchyType.None, "CabProMedEntity", New CabProMedEntityFactory())
		End Sub
		...
		''' <summary>
		''' Retrieves in the calling CabProMedCollection object all CabProMedEntity objects which have data in common
		...
		Public Overloads Function GetMulti(containingTransaction As ITransaction, collectionToFill As IEntityCollection, maxNumberOfItemsToReturn As Long, sortClauses As ISortExpression, entityFactoryToUse As IEntityFactory, Filter As IPredicateExpression, almacenInstance As IEntity, inventarioMovimientoAlmacenInstance As IEntity, pageNumber As Integer, pageSize As Integer) As Boolean
			MyBase.EntityFactoryToUse = entityFactoryToUse
			Dim fieldsToReturn  As IEntityFields = EntityFieldsFactory.CreateEntityFieldsObject(Integralab.ORM.EntityType.CabProMedEntity)
			Dim selectFilter As IPredicateExpression= CreateFilterUsingForeignKeys(almacenInstance, inventarioMovimientoAlmacenInstance, fieldsToReturn)
		...
		''' <summary>
		''' Retrieves entities of the type 'CabProMedEntity' in a datatable which match the specified filter. It will always create a new connection to the database.
		''' </summary>
		''' <param name="maxNumberOfItemsToReturn"> The maximum number of items to return with this retrieval query. 
		''' If the used Dynamic Query Engine supports it, 'TOP' is used to limit the amount of rows to return. When set to 0, no limitations are specified.</param>
		''' <param name="sortClauses">The order by specifications for the sorting of the resultset. When not specified, no sorting is applied.</param>
		...
		''' <summary>
		''' Retrieves in the calling CabProMedCollection object all CabProMedEntity objects
		...
		Public Function GetMultiUsingDetProMedCollectionViaDetDproMed(containingTransaction As ITransaction, collectionToFill As IEntityCollection, maxNumberOfItemsToReturn As Long, sortClauses As ISortExpression, entityFactoryToUse As IEntityFactory, detProMedInstance As IEntity, pageNumber As Integer, pageSize As Integer) As Boolean
			...
			relations.Add(CabProMedEntity.Relations.DetDproMedEntityUsingFolioProrrateoMedicamento, "DetDproMed_")
...
selectFilter.Add(New FieldCompareValuePredicate(detProMedInstance.Fields(CInt(DetProMedFieldIndex.IdMedicamento)), ComparisonOperator.Equal))
		...
		''' <param name="pageSize">The page size of the page to retrieve.</param>
		''' <returns>true if succeeded, false otherwise</returns>
		Public Function GetMultiUsingProductoCollectionViaDetProMed(containingTransaction As ITransaction, collectionToFill As IEntityCollection, maxNumberOfItemsToReturn As Long, sortClauses As ISortExpression, entityFactoryToUse As IEntityFactory, productoInstance As IEntity, pageNumber As Integer, pageSize As Integer) As Boolean
			Dim fieldsToReturn As IEntityFields = EntityFieldsFactory.CreateEntityFieldsObject(Integralab.ORM.EntityType.CabProMedEntity)
			Dim relations As New RelationCollection()
			relations.Add(CabProMedEntity.Relations.DetProMedEntityUsingFolioProrrateoMedicamento, "DetProMed_")
		...
		''' <summary>
		''' Deletes from the persistent storage all 'CabProMed' entities which have data in common
		''' with the specified related Entities. If one is omitted, that entity is not used as a filter. 
		''' </summary>
		''' <param name="containingTransaction">A containing transaction, if caller is added to a transaction, or Nothing if not.</param>
		''' <param name="almacenInstance">AlmacenEntity instance to use as a filter for the CabProMedEntity objects to delete</param>
		''' <param name="inventarioMovimientoAlmacenInstance">InventarioMovimientoAlmacenEntity instance to use as a filter for the CabProMedEntity objects to delete</param>
		''' <returns>Amount of entities affected, if the used persistent storage has rowcounting enabled.</returns>
		Public Overloads Function DeleteMulti(containingTransaction As ITransaction, almacenInstance As IEntity, inventarioMovimientoAlmacenInstance As IEntity) As Integer
			Dim fields As IEntityFields = EntityFieldsFactory.CreateEntityFieldsObject(Integralab.ORM.EntityType.CabProMedEntity)
			Dim deleteFilter As IPredicateExpression = CreateFilterUsingForeignKeys(almacenInstance, inventarioMovimientoAlmacenInstance, fields)
		...
		''' <param name="containingTransaction">A containing transaction, if caller is added to a transaction, or Nothing if not.</param>
		''' <param name="almacenInstance">AlmacenEntity instance to use as a filter for the CabProMedEntity objects to update</param>
		''' <param name="inventarioMovimientoAlmacenInstance">InventarioMovimientoAlmacenEntity instance to use as a filter for the CabProMedEntity objects to update</param>
		''' <returns>Amount of entities affected, if the used persistent storage has rowcounting enabled.</returns>
		Public Overloads Function UpdateMulti(entityWithNewValues As IEntity, containingTransaction As ITransaction, almacenInstance As IEntity, inventarioMovimientoAlmacenInstance As IEntity) As Integer
			Dim fields As IEntityFields = EntityFieldsFactory.CreateEntityFieldsObject(Integralab.ORM.EntityType.CabProMedEntity)
			Dim updateFilter As IPredicateExpression = CreateFilterUsingForeignKeys(almacenInstance, inventarioMovimientoAlmacenInstance, fields)
		...
		''' </summary>
		''' <param name="almacenInstance">AlmacenEntity instance to use as a filter for the CabProMedEntity objects</param>
		''' <param name="inventarioMovimientoAlmacenInstance">InventarioMovimientoAlmacenEntity instance to use as a filter for the CabProMedEntity objects</param>
		''' <param name="fieldsToReturn">IEntityFields implementation which forms the definition of the fieldset of the target entity.</param>
		''' <returns>A ready to use PredicateExpression based on the passed in foreign key value holders.</returns>
		Private Function CreateFilterUsingForeignKeys(almacenInstance As IEntity, inventarioMovimientoAlmacenInstance As IEntity, fieldsToReturn As IEntityFields) As IPredicateExpression
			Dim selectFilter As IPredicateExpression = New PredicateExpression()
			
			If Not almacenInstance Is Nothing Then
				selectFilter.Add(New FieldCompareValuePredicate(fieldsToReturn(CInt(CabProMedFieldIndex.IdAlmacen)), ComparisonOperator.Equal, CType(almacenInstance, AlmacenEntity).AlmacenId))
			End If
			If Not inventarioMovimientoAlmacenInstance Is Nothing Then
				selectFilter.Add(New FieldCompareValuePredicate(fieldsToReturn(CInt(CabProMedFieldIndex.IdAlmacen)), ComparisonOperator.Equal, CType(inventarioMovimientoAlmacenInstance, InventarioMovimientoAlmacenEntity).AlmacenId))
selectFilter.Add(New FieldCompareValuePredicate(fieldsToReturn(CInt(CabProMedFieldIndex.FolioMovimientoAlmacenSalida)), ComparisonOperator.Equal, CType(inventarioMovimientoAlmacenInstance, InventarioMovimientoAlmacenEntity).FolioMovimiento))
			End If
			Return selectFilter
		End Function
