...
Namespace Integralab.ORM.DaoClasses
	...
	''' <summary>
	''' General DAO class for the McgcatCompradoresdeGanado Entity. It will perform database oriented actions for
	''' a entity of type 'McgcatCompradoresdeGanadoEntity'. This DAO works on an EntityFields object. 
	''' </summary>
	Public Class McgcatCompradoresdeGanadoDAO 
		Inherits DaoBase
		
		''' <summary>CTor</summary>
		Public Sub New()
			MyBase.New(InheritanceInfoProviderSingleton.GetInstance(), New DynamicQueryEngine(), InheritanceHierarchyType.None, "McgcatCompradoresdeGanadoEntity", New McgcatCompradoresdeGanadoEntityFactory())
		End Sub
		...
		''' <summary>
		''' Retrieves in the calling McgcatCompradoresdeGanadoCollection object all McgcatCompradoresdeGanadoEntity objects which have data in common
		...
		Public Overloads Function GetMulti(containingTransaction As ITransaction, collectionToFill As IEntityCollection, maxNumberOfItemsToReturn As Long, sortClauses As ISortExpression, entityFactoryToUse As IEntityFactory, Filter As IPredicateExpression, cuentaContableInstance As IEntity, pageNumber As Integer, pageSize As Integer) As Boolean
			...
			Dim fieldsToReturn  As IEntityFields = EntityFieldsFactory.CreateEntityFieldsObject(Integralab.ORM.EntityType.McgcatCompradoresdeGanadoEntity)
		...
		''' <summary>
		''' Retrieves entities of the type 'McgcatCompradoresdeGanadoEntity' in a datatable which match the specified filter. It will always create a new connection to the database.
		''' </summary>
		''' <param name="maxNumberOfItemsToReturn"> The maximum number of items to return with this retrieval query. 
		''' If the used Dynamic Query Engine supports it, 'TOP' is used to limit the amount of rows to return. When set to 0, no limitations are specified.</param>
		''' <param name="sortClauses">The order by specifications for the sorting of the resultset. When not specified, no sorting is applied.</param>
		...
		''' <summary>
		''' Retrieves in the calling McgcatCompradoresdeGanadoCollection object all McgcatCompradoresdeGanadoEntity objects
		...
		Public Function GetMultiUsingCabMovGanCollectionViaCabRegCom(containingTransaction As ITransaction, collectionToFill As IEntityCollection, maxNumberOfItemsToReturn As Long, sortClauses As ISortExpression, entityFactoryToUse As IEntityFactory, cabMovGanInstance As IEntity, pageNumber As Integer, pageSize As Integer) As Boolean
			...
			relations.Add(McgcatCompradoresdeGanadoEntity.Relations.CabRegComEntityUsingCveComiGan, "CabRegCom_")
		...
		''' <summary>
		''' Deletes from the persistent storage all 'McgcatCompradoresdeGanado' entities which have data in common
		''' with the specified related Entities. If one is omitted, that entity is not used as a filter. 
		''' </summary>
		''' <param name="containingTransaction">A containing transaction, if caller is added to a transaction, or Nothing if not.</param>
		''' <param name="cuentaContableInstance">CuentaContableEntity instance to use as a filter for the McgcatCompradoresdeGanadoEntity objects to delete</param>
		...
		Public Overloads Function DeleteMulti(containingTransaction As ITransaction, cuentaContableInstance As IEntity) As Integer
			Dim fields As IEntityFields = EntityFieldsFactory.CreateEntityFieldsObject(Integralab.ORM.EntityType.McgcatCompradoresdeGanadoEntity)
		...
		''' <param name="containingTransaction">A containing transaction, if caller is added to a transaction, or Nothing if not.</param>
		''' <param name="cuentaContableInstance">CuentaContableEntity instance to use as a filter for the McgcatCompradoresdeGanadoEntity objects to update</param>
		''' <returns>Amount of entities affected, if the used persistent storage has rowcounting enabled.</returns>
		...
		''' </summary>
		''' <param name="cuentaContableInstance">CuentaContableEntity instance to use as a filter for the McgcatCompradoresdeGanadoEntity objects</param>
		...
		Private Function CreateFilterUsingForeignKeys(cuentaContableInstance As IEntity, fieldsToReturn As IEntityFields) As IPredicateExpression
			...
			If Not cuentaContableInstance Is Nothing Then
				selectFilter.Add(New FieldCompareValuePredicate(fieldsToReturn(CInt(McgcatCompradoresdeGanadoFieldIndex.IdCuentaContable)), ComparisonOperator.Equal, CType(cuentaContableInstance, CuentaContableEntity).Codigo))
			End If
			Return selectFilter
		End Function
