...
#Region "Class Member Declarations"
		...
		Private _mfacClientesDatosFiscales As Integralab.ORM.CollectionClasses.MfacClientesDatosFiscalesCollection
		Private _alwaysFetchMfacClientesDatosFiscales, _alreadyFetchedMfacClientesDatosFiscales As Boolean
...
#End Region
		...
		Protected Sub New(info As SerializationInfo, context As StreamingContext)
			...
			_mfacClientesDatosFiscales = CType(info.GetValue("_mfacClientesDatosFiscales", GetType(Integralab.ORM.CollectionClasses.MfacClientesDatosFiscalesCollection)), Integralab.ORM.CollectionClasses.MfacClientesDatosFiscalesCollection)
			_alwaysFetchMfacClientesDatosFiscales = info.GetBoolean("_alwaysFetchMfacClientesDatosFiscales")
			_alreadyFetchedMfacClientesDatosFiscales = info.GetBoolean("_alreadyFetchedMfacClientesDatosFiscales")
		...
		Protected Overrides Sub PostReadXmlFixups()
			...
			_alreadyFetchedMfacClientesDatosFiscales = (_mfacClientesDatosFiscales.Count > 0)
		...
		Public Overrides Sub GetObjectData(info As SerializationInfo, context As StreamingContext)
			...
			info.AddValue("_mfacClientesDatosFiscales", _mfacClientesDatosFiscales)
			info.AddValue("_alwaysFetchMfacClientesDatosFiscales", _alwaysFetchMfacClientesDatosFiscales)
			info.AddValue("_alreadyFetchedMfacClientesDatosFiscales", _alreadyFetchedMfacClientesDatosFiscales)
		...
		Public Overrides Overloads Sub SetRelatedEntityProperty(propertyName As String, entity As IEntity)
			Select Case propertyName
				...
				Case "MetodoDet"
					...
					End If
				Case "MfacClientesDatosFiscales"
					_alreadyFetchedMfacClientesDatosFiscales = True
					If Not entity Is Nothing Then
						Me.MfacClientesDatosFiscales.Add(CType(entity, MfacClientesDatosFiscalesEntity))
					End If
		...
		Public Overrides Overloads Sub SetRelatedEntity(relatedEntity As IEntity, fieldName As String)
			Select Case fieldName
				...
				Case "MfacClientesDatosFiscales"
					_mfacClientesDatosFiscales.Add(CType(relatedEntity, MfacClientesDatosFiscalesEntity))
					OnRelatedEntitySet(relatedEntity, fieldName)
				Case "NomCatEmpleados"
		...
		Public Overrides Overloads Sub UnsetRelatedEntity(relatedEntity As IEntity, fieldName As String, signalRelatedEntityManyToOne As Boolean)
			Select Case fieldName
				...
				Case "MfacClientesDatosFiscales"
					MyBase.PerformRelatedEntityRemoval(_mfacClientesDatosFiscales, relatedEntity, signalRelatedEntityManyToOne)
					OnRelatedEntityUnset(relatedEntity, fieldName)
				Case "NomCatEmpleados"
		...
		Public Overrides Function GetMemberEntityCollections() As List(Of IEntityCollection)
			...
			toReturn.Add(_mfacClientesDatosFiscales)
		...
		''' <summary>Retrieves all related entities of type 'MfacClientesDatosFiscalesEntity' Imports a relation of type '1:n'.</summary>
		''' <param name="forceFetch">if true, it will discard any changes currently in the collection and will rerun the complete query instead</param>
		''' <returns>Filled collection with all related entities of type 'MfacClientesDatosFiscalesEntity'</returns>
		Public Function GetMultiMfacClientesDatosFiscales(forceFetch As Boolean) As Integralab.ORM.CollectionClasses.MfacClientesDatosFiscalesCollection
			Return GetMultiMfacClientesDatosFiscales(forceFetch, _mfacClientesDatosFiscales.EntityFactoryToUse, Nothing)
		End Function
		...
		''' <param name="entityFactoryToUse">The entity factory to use for the GetMultiManyToOne() routine.</param>
		''' <returns>Filled collection with all related entities of the type constructed by the passed in entity factory</returns>
		Public Function GetMultiMfacClientesDatosFiscales(forceFetch As Boolean, entityFactoryToUse As IEntityFactory) As Integralab.ORM.CollectionClasses.MfacClientesDatosFiscalesCollection
			Return GetMultiMfacClientesDatosFiscales(forceFetch, entityFactoryToUse, Nothing)
		End Function
		...
		''' <param name="filter">Extra filter to limit the resultset.</param>
		''' <returns>Filled collection with all related entities of type 'MfacClientesDatosFiscalesEntity'</returns>
		Public Function GetMultiMfacClientesDatosFiscales(forceFetch As Boolean, Filter As IPredicateExpression) As Integralab.ORM.CollectionClasses.MfacClientesDatosFiscalesCollection
			Return GetMultiMfacClientesDatosFiscales(forceFetch, _mfacClientesDatosFiscales.EntityFactoryToUse, Filter)
		End Function
		...
		''' <param name="filter">Extra filter to limit the resultset.</param>
		''' <returns>Filled collection with all related entities of the type constructed by the passed in entity factory</returns>
		Public Overridable Function GetMultiMfacClientesDatosFiscales(forceFetch As Boolean, entityFactoryToUse As IEntityFactory, Filter As IPredicateExpression) As Integralab.ORM.CollectionClasses.MfacClientesDatosFiscalesCollection
			If ( Not _alreadyFetchedMfacClientesDatosFiscales Or forceFetch Or _alwaysFetchMfacClientesDatosFiscales) AndAlso Not MyBase.IsSerializing AndAlso Not MyBase.IsDeserializing AndAlso Not MyBase.InDesignMode Then
				If MyBase.ParticipatesInTransaction Then
					If Not _mfacClientesDatosFiscales.ParticipatesInTransaction Then
						MyBase.Transaction.Add(_mfacClientesDatosFiscales)
					End If
				End If
				_mfacClientesDatosFiscales.SuppressClearInGetMulti = Not forceFetch
				If Not entityFactoryToUse Is Nothing Then
					_mfacClientesDatosFiscales.EntityFactoryToUse = entityFactoryToUse
				End If
				_mfacClientesDatosFiscales.GetMultiManyToOne(Me, Filter)
				_mfacClientesDatosFiscales.SuppressClearInGetMulti = False
				_alreadyFetchedMfacClientesDatosFiscales = True
			End If
			Return _mfacClientesDatosFiscales
		End Function

		''' <summary>Sets the collection parameters for the collection for 'MfacClientesDatosFiscales'. These settings will be taken into account
		''' when the property MfacClientesDatosFiscales is requested or GetMultiMfacClientesDatosFiscales is called.</summary>
		''' <param name="maxNumberOfItemsToReturn"> The maximum number of items to return. When set to 0, this parameter is ignored</param>
		''' <param name="sortClauses">The order by specifications for the sorting of the resultset. When not specified (null), no sorting is applied.</param>
		Public Overridable Sub SetCollectionParametersMfacClientesDatosFiscales(maxNumberOfItemsToReturn As Long, sortClauses As ISortExpression)
			_mfacClientesDatosFiscales.SortClauses=sortClauses
			_mfacClientesDatosFiscales.MaxNumberOfItemsToReturn=maxNumberOfItemsToReturn
		End Sub
		...
		Protected Overrides Overloads Sub AddInternalsToContext()
			...
			_mfacClientesDatosFiscales.ActiveContext = MyBase.ActiveContext
		...
		Private Sub InitClassMembers()
			...
			_mfacClientesDatosFiscales = New Integralab.ORM.CollectionClasses.MfacClientesDatosFiscalesCollection(New MfacClientesDatosFiscalesEntityFactory())
			_mfacClientesDatosFiscales.SetContainingEntityInfo(Me, "CatDeptos")
			_alwaysFetchMfacClientesDatosFiscales = False
			_alreadyFetchedMfacClientesDatosFiscales = False
...
#Region "Class Property Declarations"
		...
		''' <summary>Creates a New PrefetchPathElement object which contains all the information to prefetch the related entities of type 'MfacClientesDatosFiscales' 
		''' for this entity. Add the object Returned by this property to an existing PrefetchPath2 instance.</summary>
		''' <Returns>Ready to use IPrefetchPathElement implementation.</Returns>
		Public Shared ReadOnly Property PrefetchPathMfacClientesDatosFiscales() As IPrefetchPathElement
			Get
				Return New PrefetchPathElement( New Integralab.ORM.CollectionClasses.MfacClientesDatosFiscalesCollection(), _
					CatDeptosEntity.Relations.MfacClientesDatosFiscalesEntityUsingIdDepartamento, _
					CType(Integralab.ORM.EntityType.CatDeptosEntity, Integer), CType(Integralab.ORM.EntityType.MfacClientesDatosFiscalesEntity, Integer), 0, Nothing, Nothing, Nothing, "MfacClientesDatosFiscales", SD.LLBLGen.Pro.ORMSupportClasses.RelationType.OneToMany)
			End Get
		End Property
		...
		''' <summary>Retrieves all related entities of type 'MfacClientesDatosFiscalesEntity' Imports a relation of type '1:n'.</summary>
		''' <remarks>This property is added for databinding conveniance, however it is recommeded to use the method 'GetMultiMfacClientesDatosFiscales()', because 
		''' this property is rather expensive and a method tells the user to cache the result when it has to be used more than once in the
		''' same scope.</remarks>
		Public Overridable ReadOnly Property [MfacClientesDatosFiscales]() As Integralab.ORM.CollectionClasses.MfacClientesDatosFiscalesCollection
			Get
				Return GetMultiMfacClientesDatosFiscales(False)
			End Get
		End Property

		''' <summary>Gets / sets the lazy loading flag for MfacClientesDatosFiscales. When set to true, MfacClientesDatosFiscales is always refetched from the 
		''' persistent storage. When set to false, the data is only fetched the first time MfacClientesDatosFiscales is accessed. You can always execute
		''' a forced fetch by calling GetMultiMfacClientesDatosFiscales(True).</summary>
		<Browsable(False)> _
		Public Property AlwaysFetchMfacClientesDatosFiscales As Boolean
			Get
				Return _alwaysFetchMfacClientesDatosFiscales
			End Get
			Set
				_alwaysFetchMfacClientesDatosFiscales = value
			End Set	
		End Property
