...
Namespace Integralab.ORM.CollectionClasses
	''' <summary>
	''' Collection class for storing and retrieving collections of McgconfiguracionMovimientoGanadoEntity objects. 
	''' </summary>
#If CF Then
	<SD.LLBLGen.Pro.ORMSupportClasses.Serializable()> _
	Public Class McgconfiguracionMovimientoGanadoCollection 
#Else
	<Serializable()> _
	Public Class McgconfiguracionMovimientoGanadoCollection 
#End If
		Inherits EntityCollectionBase(Of McgconfiguracionMovimientoGanadoEntity)
	
		''' <summary>CTor</summary>
		Public Sub New()
			MyBase.New(New McgconfiguracionMovimientoGanadoEntityFactory())
		End Sub

		''' <summary>CTor</summary>
		''' <param name="initialContents">The initial contents of this collection.</param>
		Public Sub New( initialContents As IList(Of McgconfiguracionMovimientoGanadoEntity))
		...
		''' <summary>Retrieves McgconfiguracionMovimientoGanadoEntity rows in a datatable which match the specified filter. It will always create a new connection to the database.</summary>
		...
		Public Shared Function GetMultiAsDataTable(selectFilter As IPredicate, maxNumberOfItemsToReturn As Long, sortClauses As ISortExpression, relations As IRelationCollection, pageNumber As Integer, pageSize As Integer) As DataTable
			Dim dao As McgconfiguracionMovimientoGanadoDAO = DAOFactory.CreateMcgconfiguracionMovimientoGanadoDAO()
			Return dao.GetMultiAsDataTable(maxNumberOfItemsToReturn, sortClauses, selectFilter, relations, pageNumber, pageSize)
		End Function


		''' <summary>Deletes from the persistent storage all McgconfiguracionMovimientoGanado entities which match with the specified filter, formulated in
		...
		''' will result in a query removing all McgconfiguracionMovimientoGanado entities from the persistent storage</param>
		...
		Public Overloads Function GetScalar(fieldIndex As McgconfiguracionMovimientoGanadoFieldIndex, aggregateToApply As AggregateFunction) As Object
		...
		''' <param name="aggregateToApply">Aggregate function to apply. </param>
		''' <Returns>the scalar value requested</Returns>
		Public Overloads Function GetScalar(fieldIndex As McgconfiguracionMovimientoGanadoFieldIndex, expressionToExecute As IExpression, aggregateToApply As AggregateFunction) As Object
		...
		''' <Returns>the scalar value requested</Returns>
		Public Overloads Function GetScalar(fieldIndex As McgconfiguracionMovimientoGanadoFieldIndex, expressionToExecute As IExpression, aggregateToApply As AggregateFunction, Filter As IPredicate) As Object
		...
		''' <Returns>the scalar value requested</Returns>
		Public Overloads Function GetScalar(fieldIndex As McgconfiguracionMovimientoGanadoFieldIndex, expressionToExecute As IExpression, aggregateToApply As AggregateFunction, Filter As IPredicate, groupByClause As IGroupByCollection) As Object
		...
		''' <param name="groupByClause">The groupby clause to apply to retrieve the scalar</param>
		''' <Returns>the scalar value requested</Returns>
		Public Overloads Overridable Function GetScalar(fieldIndex As McgconfiguracionMovimientoGanadoFieldIndex, expressionToExecute As IExpression, aggregateToApply As AggregateFunction, Filter As IPredicate, relations As IRelationCollection, groupByClause As IGroupByCollection) As Object
		...
		''' <summary>Updates in the persistent storage all entities which have data in common with the specified McgconfiguracionMovimientoGanadoEntity. 
		...
		''' <param name="entityWithNewValues">McgconfiguracionMovimientoGanadoEntity instance which holds the new values for the matching entities to update. Only
		...
		''' will result in an update action which will affect all McgconfiguracionMovimientoGanado entities.</param>
		''' <returns>Amount of entities affected, if the used persistent storage has rowcounting enabled.</returns>
		Public Overloads Function UpdateMulti(entityWithNewValues As McgconfiguracionMovimientoGanadoEntity, updateFilter As IPredicate) As Integer
			Dim dao As McgconfiguracionMovimientoGanadoDAO = DAOFactory.CreateMcgconfiguracionMovimientoGanadoDAO()
		...
		''' <param name="relations">The set of relations to walk to construct the total query.</param>
		''' <returns>Amount of entities affected, if the used persistent storage has rowcounting enabled.</returns>
		Public Overloads Function UpdateMulti(entityWithNewValues As McgconfiguracionMovimientoGanadoEntity, updateFilter As IPredicate, relations As IRelationCollection) As Integer
			Dim dao As McgconfiguracionMovimientoGanadoDAO = DAOFactory.CreateMcgconfiguracionMovimientoGanadoDAO()
